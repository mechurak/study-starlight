...
작성 전 독자·중심 질문·학습 결과를 정하고 관련 정본·공식 근거·예제를 확인한다.
작성 뒤에는 질문에 답하는 설명인지, 예제를 따라 결과를 판단할 수 있는지 의미를 리뷰한 뒤
기술 검증을 한다. 세부 순서와 판정 기준은 [작성·리뷰 절차](docs/content-authoring.md#작성과-의미-리뷰)를 따른다.
