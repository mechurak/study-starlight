...
이미 컨텍스트에 전달된 AGENTS.md는 매 요청마다 다시 읽지 않는다. 연결된 문서는 작업에 맞춰
별도로 읽고, 수정 대상 경로에 아직 읽지 않은 하위 AGENTS.md가 있으면 수정 전에 확인한다.
...
새 계획 문서 파일은 사용자가 명시적으로 요청한 경우에만 만든다. 작업이 복잡하거나 여러 단계·세션에
걸친다는 이유만으로 생성하지 않는다. 일반 작업은 편집·검증을 수행하고 결과를 대화로 보고한다.
기존 계획의 실행·재개를 요청받았다면 해당 문서를 갱신한다.

계획 파일명은 `docs/plans/0001-<slug>.md`처럼 번호를 네 자리로 쓴다. 상태·현재 위치는 각 계획에서만
관리하고 README에 전체 목록을 중복 기록하지 않는다. 번호 발급·실행 기록·완료 절차는
[계획 관리 규칙](docs/plans/README.md)을 따른다. 아래 번호 없는 명명은 학습 페이지에 적용한다.
