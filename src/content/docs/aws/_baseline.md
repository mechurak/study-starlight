# AWS 덱 기준

## 기준 시점

- 2026-09-07에 AWS 공식 사용자 가이드·요금표를 확인했다.
- AWS 상용 리전의 개인 학습용 단일 계정을 기본으로 설명한다. Organizations와 팀 운영은
  권장 인증 방식과 권한 경계를 이해하는 데 필요한 만큼만 다룬다.
- CLI 예제는 AWS CLI v2 기준이다. 콘솔의 영어 메뉴명을 병기하며 위치·문구는 바뀔 수 있다.

## 서술 규칙

- 운영 중 다시 찾을 개념 → 확인 경로 → 실수와 대응 순서로 쓴다. 서비스별 기능 나열이나 시험 요약은 피한다.
- 사람의 인증은 임시 자격증명을 우선한다. IAM User + 관리자 그룹은 학습 계정의 선택 가능한
  설정 경로이며 최소 권한이나 장기 권장 구성으로 표현하지 않는다.
- root 사용자와 AWS 계정, IAM 사용자와 역할, 인증과 권한을 구분한다.
- 비용은 사용량·크레딧 적용 전 비용·크레딧·청구액을 구분한다. Budgets는 실시간 지출 차단기가 아니다.
- Free Tier는 2025-07-15 이전 계정과 이후 계정, Free/Paid 플랜을 구분한다.
- 단가는 리전·단위·조건 없이 일반화하지 않는다. 현재 금액이 필요하면 공식 요금표에 연결한다.
- 바뀔 수 있는 동작에는 본문 가까이 공식 근거를 단다. reviewedAt은 실제 확인한 날짜만 쓴다.
- 각 학습 본문은 Thesis와 TermIntro로 열고, index는 DeckMap을 사용한다.
- 공통 예시는 사진 업로드·목록 조회·썸네일 처리 앱이다. 서비스 이름보다 실행·저장·호출·권한의 경계를 먼저 그린다.
- 관리형·서버리스를 무관리·무비용으로 표현하지 않는다. EC2도 AWS가 기반 인프라를 관리한다.
- 서비스와 리소스의 범위, S3 저장 클래스별 AZ 범위, RDS Multi-AZ 배포 유형을 구분한다.
- Lambda 동기·비동기·이벤트 소스 매핑의 재시도를 구분한다. 비동기 함수 오류의 기본 두 번 재시도를 전체에 적용하지 않는다.
- Cognito 앱 사용자·운영자 IAM 신원, JWT·임시 AWS 자격증명, 함수 호출 권한·실행 역할을 구분한다.

## 범위

- 현재 범위는 AWS 구조·관리 책임, IAM, 비용 확인·알림·리소스 정리, VPC 기초·연결 진단,
  컴퓨팅·저장소·DB 기초, Lambda·API·메시징 연결, Cognito, IaC·CI/CD·관측의 운영 흐름이다.
  콘텐츠 전송은 CloudFront·S3 OAC·캐시 진단과 CRR 기초까지 다룬다.
- VPC의 기본 예시는 상용 리전의 IPv4 네트워크다. NAT 배치는 zonal public NAT 기준임을
  명시하고 Regional NAT·IPv6의 차이를 별도로 설명한다.
- 실제 계정 생성·권한 변경·리소스 삭제는 독자가 자기 계정에서 판단해 수행하는 절차다.
- 조직 전체 거버넌스, 다중 VPC·하이브리드 네트워크 상세 설계, Terraform 입문, 자격증 대비는 현재 범위 밖이다.
- 전체 앱의 배포 실습, EKS 클러스터 운영 상세, DynamoDB 단일 테이블 고급 설계는 범위 밖이다.

## 강의 자료 반영 이력

2026-02-24 강의 정리 자료 `Samsung_20260224.pdf`(로컬 원본: 저장소 밖
`../Samsung_20260224.pdf`, 총 19쪽)를 2026-09-07에 읽고 반영했다. 쪽수는 PDF 페이지 기준이다.
손글씨 원문은 이미지로도 확인했다. 원본 PDF·페이지 캡처는 사이트에 포함하지 않고 설명과 D2를 새로 작성했다.

| 자료 쪽 | 반영 위치 | 살린 설명 |
|---|---|---|
| 1–4 | `foundations.mdx`, `compute.mdx` | 서버 → API 서비스, 서비스/리소스, 리전/AZ/엣지, 관리 책임 비교 |
| 5–6 | `iam.mdx`, `serverless.mdx` | 정책의 필드, 역할·STS, EC2 instance profile과 Lambda 실행 역할 |
| 7–8 | `storage.mdx` | EBS/S3 접근 방식, 객체 구성, DynamoDB 키·Query·인덱스·용량 |
| 9–13 | `compute.mdx`, `serverless.mdx` | VM/컨테이너/함수, 동기·이벤트 연결, Lambda 호출 세 갈래, API 요청·응답 경계 |
| 14–15 | `cognito.mdx` | User Pool/Identity Pool, 로그인 토큰과 AWS 임시 자격증명 |
| 16–18 | `operations.mdx` | 템플릿 → stack → 자원, CI/CD, 지표·로그·추적 |
| 19 | 반영할 본문 없음 | 실질적인 학습 내용이 없는 마지막 쪽 |

기존 IAM·비용·VPC의 운영 절차와 URL은 유지했다. 강의에 없는 비용·VPC 상세를 강의에서 온 것으로
표시하지 않는다. 자료의 서비스 나열은 앱 예시와 진단 질문으로 연결하고, AppSync·Kinesis 등의
단순 언급과 용량 산식 전체는 독립 장으로 확장하지 않았다.

공식 문서로 보정한 주요 지점은 S3 단일 AZ 클래스 예외, RDS Multi-AZ instance/cluster 구분,
DynamoDB의 강한 읽기·GSI 제약, 서버리스의 복수 과금 축, Lambda 소스별 재시도,
API Gateway REST/HTTP·proxy 구분, Cognito 향상된 인증 흐름, X-Ray SDK/daemon의 maintenance mode다.

2026-09-08에 사용자가 제공한 AWS Training 「실습 2: Amazon VPC 인프라 구축」 버전 7.12.4를
바탕으로 `vpc.mdx`에 실습 해설을 추가했다. 원문의 절차·스크립트·이미지는 복제하지 않고
CIDR 선택, 세 가지 요청 경로, Session Manager 조건, 첫 부팅의 네트워크 의존성과 결과 예측을
새로 서술했다. 단일 AZ·IPv4·Zonal public NAT 구성을 전제로 하며, 인스턴스 Name 태그 조건은
실습 환경 전용 제약으로 구분한다. 추가 절의 동작은 AWS 공식 문서로 확인했고 기존 장 전체의
`reviewedAt`은 유지했다.

2026-09-08에 사용자가 제공한 AWS Training 「실습 4: Amazon VPC에서 고가용성 구성」을 바탕으로
`high-availability-lab.mdx`를 추가했다. 이 장은 공통 사진 앱 예시의 예외로 인벤토리 앱을 사용하며,
전체 배포 절차 대신 ALB·리스너·대상 그룹·ASG의 관계, 시작 템플릿·프로파일의 전제,
Aurora 장애 조치와 AZ별 Zonal NAT 경로를 개념·콘솔 위치·예측·관찰로 해설한다.
원문의 클릭 절차·스크립트·이미지는 복제하지 않고 표와 D2를 새로 작성했다.
실제 역할 정책·사용자 데이터·AWS 실험 결과는 제공되지 않아 추정하거나 검증 완료로 표시하지 않는다.
IAM 장의 프로파일 확인 경로와 컴퓨팅 장의 실습 연결도 보완했으며 기존 장 전체의 reviewedAt은 유지했다.


2026-09-09에 사용자가 제공한 AWS Training 「실습 5: 서버리스 아키텍처 구축」을 바탕으로
`serverless-lab.mdx`를 추가했다. 사진 앱의 S3 → SNS → 작업별 SQS → Lambda를 통해 팬아웃,
알림과 객체 데이터의 분리, 화살표별 권한, 중첩 입력, 재시도·DLQ와 관측을 새로 설명한다.
원문의 절차·코드·이미지는 복제하지 않고 D2·표·실험 질문을 작성했다. 실제 생성 절차·코드에 맞춰
큐 두 개와 같은 버킷의 입력·출력 prefix를 기준으로 삼는다.
Standard의 순서·중복 보장 한계, 객체 key 디코딩, 테스트 이벤트, Disabled 매핑 확인 누락을 보정했다.
제공된 ZIP·실행 역할 정책과 실제 AWS 실행 결과는 확인하지 않았으며 예측과 관찰을 구분한다.
공식 문서로 새 장의 동작을 확인했고 기존 장의 reviewedAt은 유지했다.

2026-09-09에 사용자가 제공한 AWS Training 「실습 6: Amazon S3 오리진으로 Amazon CloudFront 배포 구성」을
바탕으로 `cloudfront-lab.mdx`를 추가했다. 공통 사진 앱의 전달 단계로 연결하되 제공 환경의
`CachedObjects/logo.png`를 사용하며, 이전 서버리스 실습과 자동 연결된 환경으로 표현하지 않는다.
원문의 클릭 절차·이미지는 복제하지 않고 D2·정책 예시·비교표·진단 질문을 새로 작성했다.
Behavior의 첫 일치·기본 경로, OAC와 사용자 인증의 경계, 캐시 키·갱신·오류 캐시를 설명하고
CRR은 버전 관리·권한·기존 객체·복제 상태와 자동 장애 조치의 경계까지만 다룬다.
태스크 5.2의 차단 선택 취소와 결론의 OAI 표기, 패턴 불일치 시 무조건 오류라는 설명을 보정했다.
실제 AWS 배포·요청·복제를 실행하지 않았으므로 예상 결과와 측정 결과를 구분한다.
새 장의 동작은 AWS 공식 문서로 확인했고 기존 장 전체의 `reviewedAt`은 유지했다.
