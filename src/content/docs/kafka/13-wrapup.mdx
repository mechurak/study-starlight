...
```d2 layout=elk pad=40 title="프로듀서에서 온프렘 Strimzi 위의 토픽을 거쳐 컨슈머 그룹 둘로 이어지고, KRaft 컨트롤러와 모니터링이 클러스터에 붙는 덱 전체 지도"
direction: down
...
prod: "쓰는 쪽 (4장)" {
  p: "프로듀서\nacks=all · 멱등성 · 키 선택" { class: ok }
}

k8s: 온프렘 k8s — Strimzi (9~10장) {
  cluster: "Kafka 클러스터 (3장)" {
    t: "토픽 = 파티션들의 묶음 (2 · 7장)\nRF=3 · min.insync.replicas=2\n보존: delete 또는 compact" { class: key }
  }
  cc: "컨트롤러 (KRaft) × 3" { class: mute }
  mon: "Prometheus · Grafana\n랙 · URP · 디스크" { class: mute }
}

cons: "읽는 쪽 (5~6장)" {
  g1: "컨슈머 그룹 A\n처리 후 커밋 + 멱등 처리" { class: ok }
  g2: "컨슈머 그룹 B\n(독립 오프셋)" { class: ok }
}

prod.p -> k8s.cluster.t
k8s.cluster.t -> cons.g1
k8s.cluster.t -> cons.g2
k8s.cc -> k8s.cluster.t: 리더 선출 · 메타데이터 { style.stroke-dash: 3 }
k8s.mon -> k8s.cluster.t: 감시 { style.stroke-dash: 3 }
```
