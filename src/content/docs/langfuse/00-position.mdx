...
```d2 layout=elk pad=40 title="사용자 요청이 애플리케이션과 LiteLLM을 거쳐 모델로 가는 본선 옆에서, Langfuse와 일반 관측 스택이 각각 무엇을 받아 가는지 보여주는 자리 지도"
direction: down
...
user: 사용자 요청 { class: key }
app: "LLM 애플리케이션\nRAG · agent · workflow" { class: key }
gate: "LiteLLM\n인증 · budget · routing" { class: ok }
model: "모델 endpoint\n외부 API · 사내 vLLM" { class: ok }
lf: "Langfuse\n동작 · cost · quality" { class: zone }
apm: "일반 관측 스택\nmetric · log · OTel trace" { class: warn }
loop: prompt · dataset · evaluation { class: zone }

user -> app
app -> gate
gate -> model
app -> lf { style.stroke-dash: 3 }
gate -> lf { style.stroke-dash: 3 }
app -> apm { style.stroke-dash: 3 }
lf -> loop
```
