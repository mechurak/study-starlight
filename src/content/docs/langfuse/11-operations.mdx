...
```d2 layout=elk pad=40 title="Langfuse가 애플리케이션을 관측하는 것과 별개로, 사내 OTel·APM이 Langfuse 자체의 Web·큐·Worker·저장소 health를 본다는 구분"
direction: down
...
app: LLM 애플리케이션 { class: key }
lf: "Langfuse\n동작·cost·quality" { class: key }
self: "사내 OTel/APM\nLangfuse 자체 health" { class: zone }
web: Web latency/error { class: warn }
queue: queue depth/age { class: warn }
worker: Worker throughput/error { class: warn }
store: PG · Redis · S3 · CH { class: warn }

app -> lf
lf -> self { style.stroke-dash: 3 }
self -> web
self -> queue
self -> worker
self -> store
