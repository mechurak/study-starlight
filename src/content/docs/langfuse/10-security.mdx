...
```d2 layout=elk pad=40 title="PII가 섞일 수 있는 사용자 입력이 앱 경계 masking을 지나 SDK·Web·Worker를 거쳐 ClickHouse와 UI까지 가는 동안 어디서 가려지는지 보여주는 경로"
direction: down
...
input: "사용자 input\nPII · secret 가능" { class: bad }
mask: 앱 경계 masking { class: key }
sdk: Langfuse SDK { class: zone }
web: Web ingest { class: zone }
s3: raw event bucket { class: warn }
redis: queue { class: zone }
worker: "Worker\n선택적 server masking" { class: zone }
ch: ClickHouse { class: warn }
ui: UI · API · export { class: warn }
