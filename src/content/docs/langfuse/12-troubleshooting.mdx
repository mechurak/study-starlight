...
```d2 layout=elk pad=40 title="trace가 안 보일 때 SDK 완결·전송·Web 수신·Worker 처리·ClickHouse 적재·조회 순으로 좁혀 가는 여섯 관문"
direction: down
...
make: "1. SDK가 observation을\n완결했는가" { class: key }
send: "2. batch·flush가\n전송했는가" { class: key }
accept: "3. Web이 인증하고\nS3+queue에 받았는가" { class: zone }
work: "4. Worker가\n처리했는가" { class: zone }
store: "5. ClickHouse에\n적재됐는가" { class: warn }
read: "6. API/UI filter가\n읽는가" { class: bad }
