...
```mermaid
flowchart TB
    ROOT["agent-run<br/>root span"] --> PLAN["agent: plan"]
    PLAN --> G1["generation: decide-next-action"]
    PLAN --> T1["tool: search-internal-docs"]
    PLAN --> T2["tool: fetch-ticket"]
    PLAN --> G2["generation: compose-final-answer"]
    G2 -.-> S["score: task-success"]
