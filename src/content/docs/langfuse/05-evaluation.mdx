...
```d2 layout=elk pad=40 title="production observation에 자동 평가·사용자 피드백·사람 주석 세 경로로 score가 붙고, 모니터링에서 낮은 score 사례를 골라 dataset과 오프라인 실험으로 넘기는 흐름"
direction: down
...
prod: production observation { class: key }
auto: code · LLM judge { class: zone }
user: 사용자 feedback { class: zone }
human: annotation queue { class: zone }
score: score { class: key }
mon: segment · trend · alert { class: warn }
hard: 낮은 score 사례 { class: warn }
data: dataset { class: ok }
exp: offline experiment { class: ok }
