...
```d2 layout=elk pad=40 title="인프라 버전을 올리고 서버 v4를 띄운 뒤 클라이언트 전환·이력 처리·events_only 커밋 순으로 가는 업그레이드 단계"
direction: down
...
infra: "1. Infrastructure\nCH 25.12+ · PG 15+ · Redis 7+" { class: key }
server: "2. Server v4\nlegacy 또는 dual" { class: zone }
client: 3a. SDK·API·evaluator 전환 { class: zone }
history: "3b. backfill 또는\nretention rollover" { class: warn }
cut: "3c. events_only\ncommit" { class: ok }
