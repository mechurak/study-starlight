...
> DQN의 두 처방 — **경험 재생**과 **타깃 네트워크** — 은 지금도 많은 딥 off-policy 가치·액터-크리틱 알고리즘에 쓰인다
...
그래서 Q러닝은 **현재 정책이 아닌 정책이 모은 데이터로도 갱신할 수 있다.**
예전 정책이나 사람이 만든 데이터도 후보가 된다. 다만 데이터 커버리지가 부족하면
오프라인 RL 특유의 외삽 오류가 생기므로 보정이 필요하다 ([7장](/rl/07-offline-model/)).
...
```d2 layout=elk pad=40 title="환경이 만든 전이를 재생 버퍼에 쌓고 무작위 미니배치로 Q 신경망을 학습해 ε-greedy로 다시 행동하는 DQN 학습 순환도"
direction: down
...
net: Q 신경망 학습 { class: key }

env -> buf: "(s,a,r,s') 하나씩"
buf -> net: 무작위로 32~256개
net -> env: ε-greedy 로 행동
```
...
**재생 버퍼 + 타깃 네트워크**는 이후 많은 딥 off-policy 가치·액터-크리틱 알고리즘
(TD3 · SAC 등)의 공통 부품이 됐다. 다만 모든 off-policy 방법이 두 장치를 반드시 쓰는 것은 아니다.
:::
...
**과대추정을 줄이려는 목표는 그대로 살아남았다.** 6장의 TD3는 고르기·평가 분리와는 다른
메커니즘인 "두 크리틱 중 작은 값"으로 같은 문제를 억제한다.
...
| 트레이딩 | 이산 행동이면 후보가 되지만, 과거 로그만 쓸 때는 **오프라인 보정과 강한 기준선 비교**가 필요하다 ([7장](/rl/07-offline-model/)) |
...
- [Human-level control through deep reinforcement learning](https://www.nature.com/articles/nature14236) — DQN 원 논문
- [Deep Reinforcement Learning with Double Q-learning](https://arxiv.org/abs/1509.06461) — Double DQN과 과대추정 분석
- [Reinforcement Learning: An Introduction, 2nd ed.](http://incompleteideas.net/book/the-book-2nd.html) — Q러닝, SARSA, 함수 근사와 치명적 삼요소
...
- 이 둘은 DQN을 넘어 **많은 딥 off-policy 알고리즘의 공통 부품**이 됐다
