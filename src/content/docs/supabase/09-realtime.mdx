...
```d2 layout=elk pad=40 title="Realtime 세 방식 고르기 — DB 변경인지, 구독자가 수천 명인지에 따라 Broadcast·Postgres Changes·Broadcast from Database를 나누고 접속 여부는 Presence가 맡는다"
direction: down
...
q1: "DB 에 저장되는\n데이터의\n변경인가?" { shape: diamond; class: mute }
b1: "Broadcast\n커서 · 타이핑 · 임시 알림" { class: ok }
q2: "동시 구독자가\n수천 명\n규모인가?" { shape: diamond; class: mute }
pc: "Postgres Changes\n가장 간단" { class: warn }
bd: "Broadcast from Database\n트리거 + 브로드캐스트" { class: ok }
q3: "누가 접속해\n있는지\n알아야 하나?" { shape: diamond; class: mute }
p: Presence { class: key }
