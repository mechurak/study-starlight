...
```d2 layout=elk pad=40 title="페이지가 느릴 때 네트워크 왕복·쿼리 자체·연결 대기·렌더링 넷 중 어디인지 나누고 각각의 대응을 붙인 진단표"
direction: down
...
p: 페이지가 느리다 { class: mute }
a: "어디가 느린가?" { shape: diamond; class: mute }
a1: "네트워크 왕복\n리전 불일치 · N+1" { class: mute }
a2: "쿼리 자체\n인덱스 없음 ·\nRLS 비효율" { class: key }
a3: "연결 대기\n커넥션 고갈" { class: mute }
a4: "렌더링\n함수 콜드 스타트" { class: mute }
s1: "리전 정렬 ·\n쿼리 병합" { class: mute }
s2: "EXPLAIN ANALYZE ·\n인덱스" { class: key }
s3: Supavisor · 풀 크기 { class: mute }
s4: 캐싱 · 정적화 { class: mute }
...
**원인:** 서버리스 함수가 Direct host(`db.<ref>.supabase.co:5432`) 연결을 각자 열고 있다.
...
- 마이그레이션은 Direct host(`db.<ref>.supabase.co:5432`)로 — 풀러를 거치면 일부 DDL이 실패할 수 있다
...
5. Storage에 긴 `cacheControl` — CDN 히트도 **cached egress**로 집계되지만 단가가 더 낮다
...
- [데이터베이스 연결 방식](https://supabase.com/docs/guides/database/connecting-to-postgres) — Direct와 Supavisor session/transaction host·port
- [Egress 사용량](https://supabase.com/docs/guides/platform/manage-your-usage/egress) — 일반 egress와 cached egress의 별도 쿼터·단가
- [RLS 성능 권장 사항](https://supabase.com/docs/guides/database/postgres/row-level-security#rls-performance-recommendations) — 함수 감싸기, 인덱스, 명시적 필터
- [Edge Function 한도](https://supabase.com/docs/guides/functions/limits) — CPU·메모리·wall-clock 제한
