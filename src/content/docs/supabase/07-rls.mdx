...
```d2 layout=elk pad=40 title="GRANT로 테이블 접근을 먼저 확인하고 통과하면 POLICY가 행 단위로 판정하는 두 단계 관문"
direction: down
...
r: "요청 (role: authenticated)" { class: mute }
g: "GRANT\n테이블 접근 권한이 있나?" { shape: diamond; class: mute }
d1: 거부 (42501) { class: bad }
p: "POLICY\n이 행에 접근해도 되나?" { shape: diamond; class: mute }
d2: 해당 행 제외 { class: warn }
allow: 허용 { class: ok }
...
-- 권장: 정상 클라이언트가 user_id를 생략하게 하는 편의 기본값
-- 위조 방지는 위 정책의 with check가 담당한다
...
create policy "내 글 수정"
  on public.posts for update to authenticated
  using ( (select auth.uid()) = author_id )
  with check ( (select auth.uid()) = author_id );

create policy "내 글 삭제"
...
- 추가 조회 없음 → 가장 빠르다
- [6A장의 Custom Access Token Hook](/supabase/06-auth-reference/#커스텀-클레임과-rbac)이 필요하다
...
- [ ] `to`로 대상 역할을 명시했는가
- [ ] 소유자 컬럼에 `default auth.uid()`를 두고, `with check`로 명시값 위조를 막았는가

## 참고 자료

- [Supabase Row Level Security](https://supabase.com/docs/guides/database/postgres/row-level-security) — 정책 문법, 헬퍼 함수, 성능 권장 사항
- [PostgreSQL Row Security Policies](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) — permissive/restrictive 결합과 테이블 소유자 우회
- [RLS 테스트](https://supabase.com/docs/guides/local-development/testing/pgtap-extended) — pgTAP 기반 정책 자동화
