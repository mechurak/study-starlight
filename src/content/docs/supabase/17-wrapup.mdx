...
```d2 layout=elk pad=40 title="Vercel은 실행 계층, Supabase는 상태 계층 — Auth·Storage·Realtime·Edge Functions가 모두 Postgres + RLS 위에 있다"
direction: down
...
u: 사용자 { class: mute }
v: "Vercel — 실행 계층\n렌더링 · 라우팅 · 요청 단위 서버 코드" { class: mute }

s: Supabase — 상태 계층 {
  a: "Auth\nJWT 발급" { class: mute }
  p: Postgres + RLS { shape: cylinder; class: key }
