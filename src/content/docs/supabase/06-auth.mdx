...
이 중개 구조는 keycloak 덱의 [Identity Brokering](/keycloak/identity-brokering/)과 같은 자리다.
</Card>
</CardGrid>
...
```d2 pad=40 title="Google 로그인 시 인증 결과가 Supabase callback과 앱 callback을 거쳐 code로 전달되고 세션으로 교환되는 순서"
...
g -> s: 허용된 Supabase callback으로 결과 전달 { style.stroke-dash: 3 }
s -> app: 허용된 앱 callback으로 code 전달 { style.stroke-dash: 3 }
app -> s: code를 Supabase session으로 교환
s -> app: access token + refresh token { style.stroke-dash: 3 }
```
