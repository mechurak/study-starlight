...
2. **Supabase 대시보드 → Authentication → Sign In / Providers**
   ...
   Google을 켜고 Client ID / Secret을 입력한다. Google 동의 화면이 테스트 상태라면
   등록된 테스트 사용자만 로그인할 수 있으므로 공개 전 Audience 상태도 확인한다.
...
또한 로컬 `config.toml`은 로컬 스택 설정이다. `db push`만으로 호스팅 Auth 설정이나
Google Client Secret이 배포되지는 않는다.
:::
