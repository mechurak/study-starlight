...
```d2 layout=elk pad=40 title="Supabase 아키텍처 — Envoy 게이트웨이 뒤에 PostgREST·Auth·Realtime·Storage·pg_graphql·Edge Functions가 있고 전부 PostgreSQL 위에 얹힌다"
direction: down
...
cl: "클라이언트\n브라우저 · 모바일 · 서버" { class: mute }
ev: "Envoy\nAPI 게이트웨이 · 인증 · 라우팅" { class: warn }
pr: "PostgREST\n/rest/v1" { class: mute }
gt: "Auth (GoTrue)\n/auth/v1" { class: mute }
rt: "Realtime\n/realtime/v1" { class: mute }
st: "Storage API\n/storage/v1" { class: mute }
gq: "pg_graphql\n/graphql/v1" { class: mute }
ef: "Edge Functions\n/functions/v1" { class: mute }
pg: PostgreSQL { shape: cylinder; class: key }
s3: 오브젝트 스토리지 + CDN { class: mute }
app: "외부 앱 · ORM\nPrisma · Drizzle" { class: mute }
sv: "Supavisor\n커넥션 풀러" { class: warn }
...
```d2 pad=40 title="클라이언트 요청이 Envoy와 PostgREST를 거쳐 Postgres에 닿고 RLS로 행이 걸러지는 순서"
...
c -> e: "① GET /rest/v1/posts?select=*\napikey + Authorization Bearer"
e -> e: ② apikey 검증 → 프로젝트 식별
e -> p: ③ 요청 전달
p -> p: ④ JWT 검증 → role 클레임 추출
p -> d: "⑤ set local role = 'authenticated'\nset local request.jwt.claims = '...'"
p -> d: ⑥ SELECT * FROM posts
d.note: RLS 정책이 auth.uid() 기준으로 행 필터링
d -> p: ⑦ 허용된 행만 { style.stroke-dash: 3 }
p -> c: ⑧ JSON 응답 { style.stroke-dash: 3 }
```
...
역할로 보면 GoTrue는 [keycloak 덱의 인증 생태계 계층](/keycloak/authentication-landscape/)에서
IdP(Identity Provider) 층에 해당한다 — Supabase 백엔드 전용의 작은 IdP다.
...
```d2 layout=elk pad=40 title="publishable key는 anon 또는 authenticated 역할로 RLS 평가를 받고, secret key는 service_role로 RLS를 우회한다"
direction: down
...
pk: "publishable key\nsb_publishable_..." { class: ok }
sk: "secret key\nsb_secret_..." { class: bad }
anon: "role: anon" { class: mute }
auth: "role: authenticated\nauth.uid() = 사용자 ID" { class: mute }
sr: "role: service_role" { class: mute }
rls: RLS 정책 평가 { class: key }
bypass: RLS 우회 — 전체 접근 { class: bad }
