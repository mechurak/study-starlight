...
프로젝트의 모든 HTTP 트래픽이 들어오는 단일 진입점.
...
:::caution[함정]
과거 문서와 블로그에는 **Kong**으로 나온다. 같은 역할의 게이트웨이가 Envoy로 교체된 것이다.
동작 이해에는 차이가 없지만, 검색으로 찾은 설정 파일이 안 맞을 수 있다.
:::
...
**pg_meta**는 테이블 생성, 컬럼 추가, 역할 관리 같은 **DDL 작업을 REST로** 노출하는 서버이고,
