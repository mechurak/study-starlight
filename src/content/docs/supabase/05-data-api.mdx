...
```d2 layout=elk pad=40 title="필요한 쿼리를 Data API로 표현할 수 있는지에 따라 from() · 뷰 · RPC 함수 중 하나를 고르는 판단"
direction: down
...
q: 필요한 쿼리 { class: mute }
c: "Data API 로\n표현되는가?" { shape: diamond; class: mute }
api: "supabase.from(...)" { class: ok }
v: 뷰를 만든다 { class: key }
r: RPC 함수를 만든다 { class: key }
...
- 윈도우 함수, CTE(Common Table Expression, `with` 절), 재귀 쿼리
- 서브쿼리 상관 조건
...
```sql
-- 기본값은 정상 클라이언트가 author_id를 생략하게 해준다.
-- 위조 방지는 insert/update 정책의 with check가 담당한다.
...
정확히는 `using` 절에 걸려 **행이 안 보일 때** 빈 결과가 되는 것이고,
새로 쓰는 값이 `with check`를 위반하면 조용히 넘어가지 않고 `42501` 에러가 난다 (아래 에러 코드 표).
:::
...
- `using` 때문에 수정 대상 행이 보이지 않으면 **빈 결과**, `with check` 위반은 **42501 에러**다
...
- [supabase-js 데이터 조회](https://supabase.com/docs/reference/javascript/select) — 필터, 관계 조회, 반환 타입
- [PostgREST API](https://postgrest.org/en/stable/references/api.html) — Data API가 사용하는 필터·임베딩 문법
- [Supabase 데이터베이스 함수](https://supabase.com/docs/guides/database/functions) — Data API의 경계를 RPC로 넘는 방법
