...
```d2 layout=elk pad=40 title="auth.users에서 트리거로 public.profiles가 생기고 posts는 author_id로 profiles를 참조하며 RLS는 auth.uid()로 판정한다"
direction: down
...
au: "auth.users\nSupabase 관리 · API 비노출" { shape: cylinder; class: key }
pr: "public.profiles\n내가 만든다 · API 노출" { shape: cylinder; class: ok }
po: public.posts { shape: cylinder; class: mute }

au -> pr: 가입 트리거로 행 생성
pr -> po: author_id 외래 키
au -> po: "auth.uid() 로 RLS 판정" { style.stroke-dash: 3 }
```
...
`on delete cascade` / `on delete set null` / 기본값(`no action`) —
...
`b` 단독 조회에는 안 쓰인다.
`create index concurrently`는 쓰기를 막지 않는 대신 **트랜잭션이나 파이프라인 안에서 실행할 수 없다** —
현재 Supabase CLI 마이그레이션 실행 경로에서도 거부될 수 있으므로 `db reset`과 원격 적용을 모두 시험해야 한다.
...
- [사용자 데이터 연결](https://supabase.com/docs/guides/auth/managing-user-data) — `auth.users`와 `public.profiles` 트리거 패턴
- [Postgres 인덱스](https://supabase.com/docs/guides/database/postgres/indexes) — B-tree, 부분 인덱스, concurrent 생성
- [데이터베이스 함수](https://supabase.com/docs/guides/database/functions) — RPC, `security invoker`/`security definer`, `search_path`
