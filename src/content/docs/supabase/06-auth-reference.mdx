---
title: "6A. Auth 참고서"
description: "세션과 JWT부터 로그인 방식, 메타데이터, 역할, MFA, 서버 검증까지 필요할 때 찾아보는 심화 참고서"
...
> 처음 설정하는 중이라면 [6장 Auth 설정 가이드](/supabase/06-auth/)부터 읽자. 이 페이지는 구현 중 세부 동작이 궁금할 때 찾아보는 참고서다
...
```d2 layout=elk pad=40 title="로그인으로 발급된 JWT를 PostgREST가 세션 변수에 주입하고 RLS가 읽어 판정하는 4단계"
direction: down
...
l: 로그인 { class: mute }
a: Auth 가 JWT 발급 { class: ok }
p: "PostgREST 가 풀어\n세션 변수에 주입" { class: mute }
r: RLS 가 읽고 판정 { class: key }
...
무차별 대입 방어, MFA(Multi-Factor Authentication, 다단계 인증)와 기기 관리.
...
```d2 pad=40 title="비밀번호 로그인으로 받은 access token과 refresh token이 만료 임박 시 회전되며 갱신되는 순서"
...
a -> c: access_token (1시간) + refresh_token (장기) { style.stroke-dash: 3 }
c.n1: access_token 으로 API 호출
c -> a: 만료 임박 → refresh_token 으로 갱신
a -> c: 새 access_token + 새 refresh_token { style.stroke-dash: 3 }
c.n2: refresh_token 은 1회용 — 회전된다
```
...
- **refresh token** — 길게 산다. 새 access token을 받는 데만 쓴다. **사용하면 새 것으로 교체**된다.
  다만 SSR의 동시 갱신을 견디도록 기본 10초 재사용 구간 등 제한적인 예외가 있다
- 클라이언트 SDK가 이 갱신을 자동으로 한다 (`autoRefreshToken`)
- **서버 환경에서는 자동 갱신이 안 되므로 Next.js Proxy에서 명시적으로 처리**한다 ([13장](/supabase/13-nextjs/))
...
:::caution[함정]
서버 코드(Proxy, Server Component)에서 `getSession()`이 돌려주는 `user` 객체를
...
```d2 pad=40 title="메일 확인 가입 흐름 — signUp 후 확인 메일의 링크를 눌러 code를 받고 exchangeCodeForSession으로 세션을 얻는 순서"
shape: sequence_diagram
...
a -> app: user (session 은 null) { style.stroke-dash: 3 }
app -> u: "\"메일을 확인하세요\"" { style.stroke-dash: 3 }
u -> m: 링크 클릭
m -> a: GET /auth/v1/verify?token=...
a -> u: emailRedirectTo 로 리다이렉트 (code 포함) { style.stroke-dash: 3 }
u -> app: /auth/callback?code=...
app -> a: exchangeCodeForSession(code)
a -> app: session 발급 { style.stroke-dash: 3 }
```
...
// 로그아웃 — 기본 global은 모든 기기의 세션을 종료한다
await supabase.auth.signOut()
await supabase.auth.signOut({ scope: 'local' })    // 현재 세션만
```
...
비밀번호 없는 로그인. OTP(One-Time Password)는 이름 그대로 1회용 코드다 —
이메일로 링크 또는 6자리 코드를 보낸다.
...
OAuth 설정에는 이름이 비슷한 리디렉션 주소가 두 개 있다. 이 둘을 바꿔 적는 것이 가장 흔한 실패 원인이다.

| 등록하는 곳 | 주소 | 의미 |
|---|---|---|
| Google Cloud의 Authorized redirect URI | `https://<ref>.supabase.co/auth/v1/callback` | Google이 인증 결과를 **Supabase Auth로** 돌려보냄 |
| Supabase Auth의 Redirect URLs | `https://example.com/auth/callback` | Supabase가 인증 코드를 **우리 앱으로** 돌려보냄 |

흐름은 `앱 → Google → Supabase Auth → 앱의 /auth/callback → 최종 페이지`다.

### 설정 절차

<Steps>

1. **Google Cloud에서 웹 OAuth 클라이언트 등록**

   - Authorized JavaScript origins: `https://example.com`, 로컬 OAuth도 시험하면 `http://127.0.0.1:3000`
   - Authorized redirect URI: `https://<ref>.supabase.co/auth/v1/callback`
   - 로컬 Supabase Auth도 시험하면 `http://127.0.0.1:54321/auth/v1/callback`도 추가
   - Client ID / Client Secret 발급

2. **Supabase 대시보드 → Authentication → Sign In / Providers**

   Google을 켜고 Client ID / Secret을 입력한다. 개발·공개 범위는 Google의
   Audience와 Publishing 상태, 요청 scope에 따라 달라지므로 실제 대상 계정으로 시험한다.
...
4. **로컬 OAuth가 필요하면 `config.toml`도 별도로 설정**
   ...
   프로젝트 루트 `.env`에 두 값을 두고 `supabase stop && supabase start`로 로컬 Auth를 재시작한다.
...
프로덕션은 와일드카드보다 정확한 `/auth/callback` URL을 등록한다.
`db push`만으로 호스팅 Auth 설정이나 Google Client Secret이 배포되지는 않는다.
원격 Auth 설정을 코드로 관리하려면 `config.toml`의 Secret을 환경 변수로 참조하고
연결된 프로젝트에 `supabase config push`를 실행한다 ([설정 가이드](/supabase/06-auth/#대시보드-설정을-코드로-관리하기)).
:::
...
PKCE(Proof Key for Code Exchange)가 서버 사이드 렌더링 환경의 기본 플로우다 —
브라우저가 만든 비밀값(verifier)을 증명으로 요구해, 코드만 가로채서는 세션을 얻지 못하게 한다.

```d2 pad=40 title="PKCE 흐름 — 브라우저가 만든 code_verifier를 서버가 code와 함께 교환해 세션을 쿠키에 저장하는 순서"
shape: sequence_diagram
...
p -> b: 리다이렉트 (?code=...) { style.stroke-dash: 3 }
b -> s: /auth/callback?code=...
s -> a: exchangeCodeForSession(code + code_verifier)
a -> s: session (쿠키에 저장) { style.stroke-dash: 3 }
s -> b: 리다이렉트 + Set-Cookie { style.stroke-dash: 3 }
```

- **토큰이 URL 프래그먼트에 노출되지 않는다** — implicit 플로우보다 안전하다
- 세션이 **쿠키**에 저장되어 서버에서도 읽을 수 있다
- `@supabase/ssr`을 쓰면 이게 기본 동작이다

`@supabase/ssr`의 기본 세션 쿠키는 브라우저 SDK도 토큰을 읽고 갱신해야 하므로
**`HttpOnly`가 아니다.** 모든 데이터 변경의 최종 권한 검사를 RLS와 서버에서 수행해야 하는 이유다.
브라우저 JavaScript가 전혀 세션을 다루지 않는 전통적인 서버 전용 앱만 별도 `HttpOnly` 저장 전략을 고려한다.
...
SAML 2.0 기반으로 Okta, Azure AD(현 Microsoft Entra ID), Google Workspace 등과 연동한다.
...
- **서버·Proxy에서는 `getClaims()`.** 쿠키는 위조될 수 있다
...
2. **두 callback을 뒤바꿈** — Google에는 Supabase Auth 주소, Supabase에는 앱 주소를 등록한다
3. **`next`를 그대로 redirect** — `//evil.example`까지 허용하면 오픈 리디렉션이 된다
4. **기본 SMTP로 프로덕션 운영** — 속도 제한에 걸려 가입 메일이 안 간다
5. **`user_metadata`를 권한 판단에 사용** — 사용자가 직접 고칠 수 있다
6. **서버에서 `getSession()`의 user를 신뢰** — 위조 가능하다
7. **`profiles` 행이 없는 사용자** — 트리거를 안 걸었거나 실패했을 때. 트리거 실패는 가입 실패로 이어진다
8. **secret key 노출** — `NEXT_PUBLIC_` 접두사 실수, 클라이언트 컴포넌트에서 import
9. **JWT 만료 시간을 길게 설정** — 로그아웃과 정지가 즉시 반영되지 않는다
10. **이메일 확인 없이 운영** — 스팸 가입과 타인 이메일 도용의 통로
...
- [사용자 세션](https://supabase.com/docs/guides/auth/sessions) — access/refresh token, 재사용 구간, `HttpOnly` 쿠키의 제약
- [로그아웃 범위](https://supabase.com/docs/guides/auth/signout) — `global`(기본), `local`, `others`
- [SSR 고급 가이드](https://supabase.com/docs/guides/auth/server-side/advanced-guide) — PKCE와 쿠키 기반 세션 갱신
- [Google 로그인](https://supabase.com/docs/guides/auth/social-login/auth-google) — Google Cloud, 호스팅·로컬 provider, PKCE 콜백 설정
- [Redirect URL 설정](https://supabase.com/docs/guides/auth/redirect-urls) — Site URL, 정확한 프로덕션 주소, Vercel 프리뷰 와일드카드
- [JWT 서명 키](https://supabase.com/docs/guides/auth/signing-keys) — 비대칭키와 JWKS 검증
