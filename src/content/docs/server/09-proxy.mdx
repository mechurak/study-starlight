...
```d2 layout=elk pad=40 title="환경변수·apt·snap·systemd·git·CA 저장소가 각각 어떤 프로그램의 프록시와 TLS 검증을 담당하는지 짝지은 표"
direction: right
...
env: "환경변수\nhttp_proxy · no_proxy" { class: key }
apt: "/etc/apt/apt.conf.d/95proxy" { class: warn }
snap: "snap set system proxy.*" { class: warn }
unit: "systemd drop-in\nEnvironment=" { class: warn }
git: "git config http.proxy" { class: warn }
ca: "/usr/local/share/ca-certificates\n+ update-ca-certificates" { class: key }

c1: "curl · wget · pip · npm\n대부분의 CLI" { class: mute }
c2: apt { class: mute }
c3: snap { class: mute }
c4: "서비스 데몬\n(docker · 앱)" { class: mute }
c5: "git (https)" { class: mute }
c6: TLS 검증 — 전부 { class: mute }
