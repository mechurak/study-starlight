...
<TermIntro
  terms={[
    ...
    ['구조화 로그', 'Structured Logging', '사람이 읽는 문장 대신 **JSON 같은 기계가 파싱할 수 있는 형식**으로 찍는 로그. 라벨을 절제할 수 있게 해 주는 전제 조건이다.'],
...
로그가 안 지워지는 원인 1번은 **`compactor.retention_enabled`가 꺼져 있는 것**이다.
