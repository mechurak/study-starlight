...
```d2 layout=elk pad=40 title="파드 stdout이 Alloy를 거쳐 Loki distributor·ingester로 들어가 오브젝트 스토리지에 쌓이고, querier가 ingester와 스토리지를 함께 읽는 구조"
direction: down
...
pod: 파드 stdout { class: mute }
alloy: "Grafana Alloy\n(DaemonSet)" { class: key }
dist: Loki distributor { class: mute }
ing: "ingester\n(메모리 + WAL)" { class: mute }
s3: "오브젝트 스토리지\nchunk + 인덱스" {
  shape: cylinder
  class: ok
}
q: Grafana / LogQL { class: mute }
qf: querier { class: mute }
