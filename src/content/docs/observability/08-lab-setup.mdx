...
Docker·kind·kubectl을 처음 준비하거나 macOS와 Ubuntu의 차이를 확인해야 한다면
[실습 환경의 kind 장](/lab-environment/01-kind/)을 먼저 본다. 그 장의 예시 `study` 클러스터를
미리 만들 필요는 없다. 여기서는 `make up`이 전용 `observability-lab` 클러스터를 만든다.
...
- [실습 환경 — kind](/lab-environment/01-kind/) — macOS·Ubuntu 공통 준비와 클러스터 수명주기
