...
Docker·kind·kubectl을 처음 준비한다면 운영체제에 맞춰 [macOS의 kind 장](/lab-environment/01-kind-macos/)
또는 [Ubuntu의 kind 장](/lab-environment/02-kind-ubuntu/)을 먼저 본다. 그 장의 예시 `study`
클러스터를 미리 만들 필요는 없다. 여기서는 `make up`이 전용 `observability-lab` 클러스터를 만든다.

이 실습은 Docker 런타임에 **CPU 4개 · 메모리 6 GiB 이상**을 할당한다. 이미지와 클러스터를 합쳐
...
Colima와 Docker Desktop은 런타임에 메모리를 명시해 배정하고, Ubuntu의 Docker Engine은 호스트
자원을 쓴다. 어느 환경이든 다음 값에서 메모리가 6 GiB 이상인지 확인한다.
...
- [실습 환경 — kind on macOS](/lab-environment/01-kind-macos/) — Colima 또는 Docker Desktop에서 준비와 정리
- [실습 환경 — kind on Ubuntu](/lab-environment/02-kind-ubuntu/) — Docker Engine에서 준비와 정리
