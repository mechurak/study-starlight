...
<TermIntro
  terms={[
    ['Grafana', '', '메트릭·로그·트레이스를 저장하는 곳이 아니라, **세 저장소를 한 시간축에서 조회하고 서로 잇는 창**이다.'],
  ]}
/>
...
<SourceFigure
  src="/images/observability/grafana-explore-split.png"
  alt="Grafana Explore에서 두 질의를 좌우 분할 화면으로 열고 같은 시간 범위에서 비교하는 공식 예시 화면"
  width={1746}
  height={850}
  caption="Explore의 Split은 두 화면의 시간 범위를 묶는다. 이 덱에서는 왼쪽에 메트릭, 오른쪽에 로그를 두고 같은 순간을 비교한다."
  sourceHref="https://grafana.com/docs/grafana/latest/visualizations/explore/get-started-with-explore/"
  sourceLabel="Grafana 공식 문서 — Get started with Explore"
/>
...
<TermIntro
  title="연결 설정에 필요한 말"
  terms={[
    ['데이터소스', 'Data Source', 'Grafana가 붙는 저장소 하나. Prometheus · Loki · Tempo가 각각 하나이며 고유한 `uid`로 서로를 참조한다.'],
    ['프로비저닝', 'Provisioning', '데이터소스·대시보드를 **파일로 미리 넣어 두는 것**. UI에서 손으로 만든 설정과 달리 Git으로 관리할 수 있다.'],
    ['derived field', '파생 필드', '로그에서 `trace_id` 같은 값을 뽑아 **다른 데이터소스로 가는 링크**로 만드는 기능.'],
  ]}
/>

```d2 layout=elk pad=40 title="Grafana에서 Prometheus·Loki·Tempo를 exemplar·derived field·trace to logs·span metrics로 서로 잇는 네 갈래 연결"
direction: down
...
<TermIntro
  title="대시보드와 구분할 말"
  terms={[
    ['Explore', '탐색', '저장된 대시보드가 아니라 **질의를 그때그때 던지고 결과를 비교하는 화면**. 사고 조사는 대부분 여기서 한다.'],
  ]}
/>
...
<TermIntro
  title="대시보드를 줄이는 장치"
  terms={[
    ['템플릿 변수', 'Template Variable', '대시보드 위쪽의 드롭다운. `$service` 값을 바꾸며 **여러 서비스가 한 대시보드를 공유**하게 한다.'],
  ]}
/>
...
| 확인할 것 | 왜 |
|---|---|
| `groups` claim이 실제로 오는가 | 안 오면 전원이 Viewer가 된다 → [Keycloak Scope와 Mapper](/keycloak/scopes-and-mappers/) |
...
<TermIntro
  title="여기서 비교하는 기능"
  terms={[
    ['Unified Alerting', '통합 알림', 'Grafana가 직접 알림 규칙을 평가하고 보내는 기능. Alertmanager와 역할이 겹치므로 한쪽을 주인으로 정한다.'],
  ]}
/>
