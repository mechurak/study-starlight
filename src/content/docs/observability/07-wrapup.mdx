...
```d2 layout=dagre pad=40 title="수집(Alloy·Prometheus scrape) · 저장(Prometheus·Loki·Tempo) · 출구(Grafana·Alertmanager)로 묶은 덱 전체 지도와 세 신호를 잇는 점선"
direction: down
...
get: 수집 — 1장 {
  ...
  scrape: Prometheus scrape { class: mute }
}

keep: 저장 {
  prom: Prometheus — 2장 { class: mute }
  loki: Loki — 3장 { class: mute }
  tempo: Tempo — 4장 { class: mute }
}

out: 출구 {
  graf: Grafana — 5장 { class: key }
  am: Alertmanager — 2장 { class: warn }
}
...
get.alloy -> keep.loki
get.alloy -> keep.tempo
get.scrape -> keep.prom
keep.loki -> s3
keep.tempo -> s3
keep.prom -> out.am
out.graf -> keep.prom
out.graf -> keep.loki
out.graf -> keep.tempo
keep.loki -> keep.tempo: trace_id { style.stroke-dash: 3 }
keep.prom -> keep.tempo: exemplar { style.stroke-dash: 3 }
keep.tempo -> keep.loki: trace to logs { style.stroke-dash: 3 }
```
...
4. **로그** ([3장](/observability/03-loki/))
   ...
   - `ingestion_rate_mb` · `max_global_streams_per_user`로 폭주 앱 방어선을 친다
...
공식 로컬 스택을 띄우고 [9장 공식 앱 조사](/observability/09-first-investigation/)로 오류를 따라간 뒤,
[10장 첫 대시보드](/observability/10-first-dashboard/)에서 검증한 query를 반복해서 볼 화면으로 남긴다.
마지막으로 [11장 내 앱 계측](/observability/11-instrument-your-app/)에서 자체 앱에 계측을
직접 붙여 같은 스택에 연결한다 — "내 서비스는 어떻게 연결하나"의 답이 거기 있다.
