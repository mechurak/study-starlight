...
<TermIntro
  terms={[
    ...
    ['RED · USE', '', '무엇을 재야 하는지의 두 관례. RED는 서비스를 **요청 수 · 에러 · 지연**으로, USE는 자원을 **사용률 · 포화 · 오류**로 본다. 대시보드와 알림 설계의 출발점이다.'],
...
| **gauge** | 오르내린다 | `..._memory_bytes` · `kube_pod_status_ready` | 그대로 본다. 추세는 `deriv()`·`predict_linear()` |
...
node_load5 / count by (instance) (node_cpu_seconds_total{mode="idle"})             # Saturation
