...
먼저 **Code** 모드에서 서비스의 로그 스트림만 고른다.

```text
{service_name="checkout-lab"}
```

**Run query**를 눌러 정상 로그와 오류 로그가 함께 나오는지 본다. 이제 같은 query에 라인 필터를
붙여 재고 예약 timeout만 남긴다.
...
Grafana는 상세 필드를 **Indexed labels**와 **Structured metadata**로 나눠 보여 준다.
`service_name`은 앞쪽에, 요청마다 바뀌는 `trace_id`와 `span_id`는 뒤쪽에 있어야 한다.
오류 한 줄의 **Show context**를 열면 같은 시각 전후의 정상 요청도 함께 볼 수 있다.
...
아직 `make down`을 실행하지 않는다. 다음 장에서는 방금 Explore에서 검증한 query를 사용해
Grafana 대시보드를 직접 만든다.

여기서 실습을 끝낸다면 port-forward 터미널에서 `Ctrl+C`를 누른 뒤 다음 명령으로 전용 클러스터를 지운다.

```bash
make down
```

`observability-lab` kind 클러스터 전체와 `emptyDir`의 데이터가 사라진다. 다른 kind 클러스터와
Docker 이미지는 건드리지 않는다.
...
- [Grafana Logs in Explore](https://grafana.com/docs/grafana/latest/visualizations/explore/logs-integration/) — 로그 상세 필드, context, 데이터 링크를 확인하는 공식 UI 흐름
...
<LinkCard
  title="10. 첫 대시보드"
  description="Explore에서 검증한 요청량·에러율·p99·오류 로그 query를 반복해서 볼 화면으로 남긴다."
