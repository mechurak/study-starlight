...
```d2 layout=elk pad=40 title="5xx 알림에서 메트릭 그래프로 서비스·시각을 좁히고 로그와 트레이스로 갈라져 원인에 이르는 흐름"
direction: down
...
```d2 layout=elk pad=40 title="앱과 쿠버네티스에서 나온 신호가 Alloy와 Prometheus를 거쳐 Loki·Tempo·TSDB에 저장되고 Grafana와 Alertmanager로 이어지는 LGTM 스택 구성도"
direction: down
...
src: 대상 {
  app: "앱 (OTel SDK)" { class: mute }
  k8s: 노드 · 파드 · 컨트롤 플레인 { class: mute }
}

col: 수집 {
  alloy: "Grafana Alloy\n(DaemonSet + Deployment)" { class: mute }
}
...
src.app -> col.alloy
src.k8s -> col.alloy
col.alloy -> store.loki: 로그
col.alloy -> store.tempo: "트레이스 (OTLP)"
store.prom -> src.k8s: "긁어 간다 (pull)"
store.prom -> src.app: "긁어 간다 (pull)"
store.loki -> s3
store.tempo -> s3
store.prom -> am
graf -> store.prom
