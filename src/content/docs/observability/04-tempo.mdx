...
<SourceFigure
  src="/images/observability/tempo-trace-explore.png"
  alt="Grafana Explore의 로그 결과에서 TraceID를 찾아 Tempo 트레이스를 열고, 여러 서비스의 스팬을 가로 막대로 보는 공식 예시 화면"
  width={1474}
  height={788}
  caption="왼쪽 로그의 TraceID가 오른쪽 요청 여정으로 이어진다. 오른쪽 가로 막대 하나가 스팬이고, 긴 막대가 시간을 많이 쓴 구간이다."
  sourceHref="https://grafana.com/docs/tempo/latest/introduction/"
  sourceLabel="Grafana Tempo 공식 문서 — Introduction"
/>

## 스팬 하나는 어떻게 생겼나

<TermIntro
  title="스팬 안의 두 종류 꼬리표"
  terms={[
    ['리소스 속성', 'Resource Attributes', '어느 서비스·파드인가처럼 **프로세스 전체에 붙는 꼬리표**.'],
    ['스팬 속성', 'Span Attributes', '어떤 URL·SQL인가처럼 **그 구간에만 붙는 꼬리표**.'],
  ]}
/>
...
<TermIntro
  title="요청을 끊기지 않게 잇는 말"
  terms={[
    ['컨텍스트 전파', 'Context Propagation', '서비스 A가 B를 부를 때 **같은 트레이스 ID를 HTTP 헤더로 넘기는 것**. 끊기면 요청 여정도 둘로 갈라진다.'],
    ['OTLP', 'OpenTelemetry Protocol', '앱과 수집기가 트레이스를 주고받는 표준 전송 방식. 저장소를 바꿔도 앱 계측을 유지하게 해 준다.'],
  ]}
/>
...
```d2 layout=elk pad=40 title="요청이 서비스 A·B·C를 traceparent 헤더로 타고 가는 동안 각 서비스가 스팬을 Collector로 보내고 Tempo를 거쳐 오브젝트 스토리지에 저장되는 흐름"
direction: down
...
u: 요청 { class: mute }
a: "서비스 A\ntrace_id 생성" { class: bad }
b: 서비스 B { class: bad }
c: 서비스 C { class: bad }
col: "OTel Collector\n또는 Alloy" { class: mute }
t: Tempo { class: mute }
s3: 오브젝트 스토리지 — 온프렘 덱 6장 {
  shape: cylinder
  class: ok
}
...
a -> b: traceparent 헤더
b -> c: traceparent 헤더
a -> col: 스팬 { style.stroke-dash: 3 }
b -> col: 스팬 { style.stroke-dash: 3 }
c -> col: 스팬 { style.stroke-dash: 3 }
col -> t: OTLP
t -> s3
```
...
<TermIntro
  title="이 절의 핵심 용어"
  terms={[
    ['샘플링', 'Sampling', '모든 요청을 저장하지 않고 **일부만 남기는 것**. 시작할 때 정하는 방식과 요청이 끝난 뒤 정하는 방식이 있다.'],
  ]}
/>
...
<TermIntro
  title="트레이스에서 덤으로 얻는 것"
  terms={[
    ['서비스 그래프', 'Service Graph', '트레이스에서 자동 생성한 **누가 누구를 부르는가** 지도. 문서와 다른 실제 호출 관계가 드러난다.'],
  ]}
/>
...
```d2 layout=elk pad=40 title="메트릭의 exemplar로 트레이스에 들어가고, 트레이스에서 로그로, 로그의 trace_id로 다시 트레이스로 돌아오는 세 신호의 왕복"
direction: down
...
m: "메트릭\np99가 튀었다" { class: mute }
t: "트레이스\n그 순간의 느린 요청" { class: key }
l: "로그\n그 서비스의 그 시각" { class: mute }
