...
기본 경로는 공유 지침 → 지침 탐색 → 계획 문서의 생성·관리 기준 → 실행과 재개이며 monorepo는 선택 심화다.
...
2026-09-19에는 OpenAI의 AGENTS 자동 탐색 범위를 재확인하고 저장소의 계획 생성·번호·검색 규칙과
요청 예시를 갱신했다. 나머지 제품 동작의 확인 기준일은 위 날짜를 유지한다.
...
작은 작업에 PRD·changelog·decision 문서를 일괄 생성하지 않는다. 새 계획 파일은 명시적인 요청이
있을 때만 만들고 기존 계획의 실행·재개 요청은 해당 문서에 기록한다. 파일명은 네 자리 번호를 쓰고
README에 전체 목록을 복제하지 않는다. 세부 기준은 docs/plans/README.md를 따른다.
일반 요청 예시에 이미 로드된 AGENTS를 다시 읽으라는 문구를 반복하지 않는다. 하위 지침·연결 문서의
읽기는 공유 지침이 맡고, 적용 지침 확인 요청은 누락 진단 상황으로 구분한다.
