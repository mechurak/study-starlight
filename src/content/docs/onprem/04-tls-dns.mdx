...
```d2 layout=elk pad=40 title="공인 도메인이 아니면 사내 CA로 가고 신뢰 배포가 숙제로 남으며, 공인 도메인이면 :80 개방 여부에 따라 ACME HTTP-01과 DNS-01로 갈리는 인증서 발급 분기도"
direction: down
...
h1: ACME HTTP-01 { class: ok }
d1: "ACME DNS-01\n(TXT 레코드로 증명)" { class: ok }
t: 신뢰 배포가 숙제로 남는다 { class: warn }
...
```d2 layout=elk pad=40 title="cert-manager·external-dns·Argo CD의 외부 통신이 사내 egress 프록시를 거쳐 인터넷으로 나가고, 클러스터 내부 주소는 NO_PROXY로 프록시를 우회하는 구조"
direction: right
...
cluster: 클러스터 {
  grid-rows: 3
  cm: cert-manager { class: mute }
  ed: external-dns { class: mute }
  argo: Argo CD { class: mute }
}
proxy: 사내 egress 프록시 { class: warn }
net: "인터넷\nACME · DNS API · Git" { class: mute }
k8s: "클러스터 내부\nAPI 서버 · Service" { class: mute }

cluster.cm -> proxy
cluster.ed -> proxy
cluster.argo -> proxy
proxy -> net
cluster.cm -> k8s: NO_PROXY 대상 { style.stroke-dash: 3 }
```
