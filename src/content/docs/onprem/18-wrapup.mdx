...
flowchart LR
    ...
    subgraph L4["④ 운영 도구"]
        direction TB
        ARGO["Argo CD · 시크릿 — 13~14장"]
        OBS["Prometheus · Loki · Tempo · Grafana — 8~12장"]
        VEL["Velero · etcd 스냅샷 — 15장"]
    end
    subgraph L3["③ 상태"]
        direction TB
        S3[("오브젝트 스토리지 — 6장")]
...
| [8](/onprem/08-observability/) | 세 신호는 **얼마나 · 무슨 일이 · 어디서**에 답하고, 저장소는 **S3 하나로** 모인다 |
| [9](/onprem/09-prometheus/) | **알림이 없는 메트릭은 부검 자료**다. Watchdog을 외부로 라우팅한다 |
| [10](/onprem/10-loki/) | Loki는 **라벨만 인덱싱**한다 — 그래서 싸고, 그래서 **라벨 설계가 전부**다 |
| [11](/onprem/11-tempo/) | 추적은 **앱을 고쳐야** 시작된다. 가장 싼 첫걸음은 **로그에 `trace_id`** |
| [12](/onprem/12-grafana/) | Grafana의 값은 대시보드가 아니라 **신호 사이의 이동을 없애는 것**이다 |
| [13](/onprem/13-gitops/) | "뭐가 깔려 있죠?"에 **Git 주소로** 답한다. 순서는 sync wave가 인코딩한다 |
| [14](/onprem/14-secrets/) | 시크릿에는 **네 문제**가 섞여 있다 — 저장 · NS 경계 · 갱신 · 복구. **마스터키 백업이 곧 복구 가능성** |
| [15](/onprem/15-backup/) | GitOps는 **절반만** 백업한다. **복구는 리허설한 것만** 된다 |
| [16](/onprem/16-ops/) | 운영의 8할은 **디스크 · 인증서 · 버전**. 진단은 **층을 위에서 아래로** |
...
4. 되돌려야 하면 **새 Cluster로 PITR** — 원본은 건드리지 않는다 ([15장](/onprem/15-backup/))
</Card>
<Card title="로그·메트릭이 안 보인다" icon="warning">
1. **관측 스택 자체가 살아 있나** — 그래프가 평평한 것과 정상은 다르다 ([16장](/onprem/16-ops/))
...
2. `argocd app diff <app>` — 무엇이 다른가 ([13장](/onprem/13-gitops/))
...
| 메시징 | [Kafka 덱](/kafka/) | Tempo 3.0의 인입 경로에도 등장한다 |
