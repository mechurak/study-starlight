...
```d2 layout=elk pad=40 title="kubectl과 노드가 VIP :6443을 거쳐 세 대의 control-plane에 닿고, 세 control-plane의 etcd가 서로 raft 합의로 묶이는 고가용성 구성"
direction: down
...
kubectl: kubectl · 노드 · 컨트롤러 { class: mute }
vip: "VIP :6443\n(kube-vip 또는 HAProxy+keepalived)" { class: key }
cp1: "control-plane-1\napi + etcd" { class: mute }
cp2: "control-plane-2\napi + etcd" { class: mute }
cp3: "control-plane-3\napi + etcd" { class: mute }
