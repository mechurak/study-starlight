...
| **연 1회** | 사내 CA·클러스터 인증서 만료 점검, DR(Disaster Recovery, 재해 복구) 시나리오 전면 훈련, 용량 증설 예산 |
...
| Keycloak 서명 키 | 정책에 따라 | 롤오버 ([Keycloak 관리 권한과 서명 키](/keycloak/administration-and-keys/)) | 토큰 검증 실패 |
| 서비스 계정 토큰(외부 시스템용) | 설정에 따라 | 재발급 | CI(지속적 통합)·연동 실패 |
...
```d2 layout=elk pad=40 title="앱이 응답하는지에서 시작해 입구까지 오는지, 노드·CNI가 정상인지, DB·스토리지가 정상인지 순으로 층을 내려가며 원인을 좁히는 진단 사다리"
direction: down
...
client: 클라이언트 · DNS · 사용자 환경 { class: mute }
