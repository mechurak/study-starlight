...
| Secrets Manager · KMS | 비밀 저장·전달·회전 | **Sealed Secrets** 또는 ESO + 금고 | [14장](/onprem/14-secrets/) |
| AWS Backup · 스냅샷 | 백업·복구 | **Velero** + etcd 스냅샷 + CNPG PITR | [15장](/onprem/15-backup/) |
| 오토스케일링 그룹 | 노드 자동 증설 | **없다** — 용량은 사람이 미리 산다 | [16장](/onprem/16-ops/) |
| 사업자의 SLA | 가용성 책임 | **내가 정한 SLO와 온콜** | [16장](/onprem/16-ops/) |

:::tip[핵심 원칙]
빈칸을 채우는 순서는 **아래에서 위로**다. 스토리지가 없으면 DB를 못 올리고,
DB가 없으면 Keycloak을 못 올리고, Keycloak이 없으면 Grafana에 SSO를 못 붙인다.
[18장](/onprem/18-wrapup/)의 구축 체크리스트가 이 순서를 그대로 따른다.
:::
...
이 그림이 [16장](/onprem/16-ops/) 진단 사다리의 원본이다.
...
디스크가 차면 관측 스택이 먼저 죽는다 ([16장](/onprem/16-ops/)).
</Card>
