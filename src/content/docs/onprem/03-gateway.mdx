...
```d2 layout=elk pad=40 title="인프라팀이 GatewayClass와 Gateway를 만들고 앱팀이 HTTPRoute를 만들며, Gateway의 allowedRoutes가 어느 네임스페이스의 Route를 붙일지 정하는 역할 분담"
direction: right
...
infra: 인프라팀이 만든다 {
  grid-rows: 2
  gc: "GatewayClass\n어떤 구현체를 쓰나" { class: zone }
  gw: "Gateway\n리스너 · 포트 · TLS 인증서" { class: zone }
  gc -> gw
}
app: 앱팀이 만든다 {
  grid-rows: 2
  r1: "HTTPRoute\napp.example.com → svc-a" { class: ok }
  r2: "HTTPRoute\n/api → svc-b" { class: ok }
}

infra.gw -> app.r1: "allowedRoutes\n(네임스페이스 허용)"
infra.gw -> app.r2
```
...
```d2 layout=elk pad=40 title="사용자 요청이 사내 DNS 해석과 MetalLB가 광고한 IP를 거쳐 리더 노드와 Gateway 파드에서 TLS를 끝내고, 인증이 필요하면 Keycloak 로그인을 돈 뒤 Service와 Pod에 닿는 경로"
direction: down
...
u: "사용자\nhttps://grafana.example.internal" { class: mute }
dns: "사내 DNS\n(external-dns가 등록 — 4장)" { class: mute }
ip: "192.168.10.240\n(MetalLB가 광고한 IP)" { class: mute }
node: "리더 노드 NIC\nkube-proxy DNAT" { class: mute }
gw: "Gateway 파드\nTLS 종료 (cert-manager 인증서 — 4장)" { class: key }
auth: "인증 필요?\n(oauth2-proxy — 5장)" {
  shape: diamond
  class: warn
}
kc: Keycloak 로그인 { class: warn }
svc: Service → Pod { class: mute }
