...
<TermIntro
  terms={[
    ...
    ['S3 API', 'Simple Storage Service API', 'AWS S3가 만든 오브젝트 스토리지 HTTP 인터페이스. 사실상 표준이라 **거의 모든 도구가 이 API로 말한다** — 그래서 온프렘에서도 S3 호환 제품을 세운다.'],
...
| 도구 | S3에 무엇을 넣나 | 없으면 |
|---|---|---|
| **Loki** | 로그 chunk와 인덱스 | 로그를 며칠치밖에 못 들고 있다 ([관측 덱 3장](/observability/03-loki/)) |
| **Tempo** | 트레이스 블록 | 추적을 아예 못 켠다 ([관측 덱 4장](/observability/04-tempo/)) |
| **CloudNativePG** | 베이스 백업 · WAL 아카이브 | **시점 복구(PITR)가 불가능하다** ([7장](/onprem/07-cnpg/)) |
| **Velero** | 클러스터 백업 | 클러스터를 다시 못 세운다 ([11장](/onprem/11-backup/)) |
