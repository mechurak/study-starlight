...
```d2 layout=elk pad=40 title="오브젝트 스토리지를 클러스터 안에 두면 클러스터가 죽을 때 로그 저장소도 같이 죽고, 클러스터 밖 전용 스토리지 노드에 두면 S3 API로만 의존하는 대비"
grid-columns: 2
