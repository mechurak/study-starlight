...
```d2 layout=elk pad=40 title="Cluster CRD를 받은 CNPG 오퍼레이터가 primary와 replica 둘을 만들고, primary가 WAL 스트리밍 복제와 오브젝트 스토리지 백업을 하며 이상 시 replica가 승격되는 구조"
direction: down
...
spec: "Cluster CRD\ninstances: 3\nbackup · monitoring" { class: mute }
op: CNPG 오퍼레이터 { class: key }
p: "primary\n(쓰기)" { class: mute }
r1: replica 1 { class: mute }
r2: replica 2 { class: mute }
s3: "오브젝트 스토리지\n6장" {
  shape: cylinder
  class: ok
}
...
"나중에 볼 일"이 아니라 **즉시 알림 대상**이다 ([관측 덱 2장](/observability/02-prometheus/)).
:::
...
```d2 layout=elk pad=40 title="주기적 베이스 백업과 연속 WAL 아카이브를 오브젝트 스토리지에 두고, 새 Cluster의 bootstrap.recovery가 둘을 읽어 targetTime 시점의 DB로 복구하는 흐름"
direction: down
...
pg: Postgres { class: mute }
b1: "s3://pg-backups/…/base" {
  shape: cylinder
  class: ok
}
b2: "s3://pg-backups/…/wals" {
  shape: cylinder
  class: ok
}
rec: "새 Cluster\nbootstrap.recovery" { class: mute }
pit: 원하는 시점의 DB { class: key }

pg -> b1: 주기적 베이스 백업
pg -> b2: WAL 아카이브 (연속)
b1 -> rec
b2 -> rec
rec -> pit: targetTime 지정
```
...
행 수를 세어 본다.** 안 해 본 백업은 백업이 아니다 ([11장](/onprem/11-backup/)).
:::
...
| 사용자 | 무엇을 넣나 | 없으면 |
|---|---|---|
| **Keycloak** | 사용자 · 세션 · client 설정 | SSO가 통째로 멈춘다 ([Keycloak 배포 설계](/keycloak/deployment/)) |
| **Grafana** | 대시보드 · 사용자 · 알림 규칙 | SQLite면 **HA가 불가능**하다 ([관측 덱 5장](/observability/05-grafana/)) |
| 사내 앱 | 업무 데이터 | |
