...
<TermIntro
  terms={[
    ...
    ['chunk', '청크', '한 스트림의 로그를 시간·크기 단위로 뭉쳐 압축한 덩어리. **이 덩어리가 오브젝트 스토리지로 간다**(6장).'],
...
([12장](/onprem/12-grafana/)). 앱 로그에 trace ID를 넣는 것이
[11장](/onprem/11-tempo/)에서 가장 값싸게 얻는 이득이다.
...
| chunk가 안 올라감 | S3 자격증명·path-style·CA | [6장](/onprem/06-minio/) 함정 넷 |
