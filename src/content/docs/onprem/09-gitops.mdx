...
```d2 layout=elk pad=40 title="사람이 PR과 머지로 Git 저장소를 고치면 Argo CD가 주기적으로 읽어 차이를 클러스터에 적용하고, 실제 상태 보고와 드리프트 알림이 되돌아오는 GitOps 순환"
direction: right
...
k8s: 클러스터 { class: mute }
obs: Prometheus — 관측 덱 2장 { class: mute }

dev -> git: PR · 머지
argo -> git: 주기적으로 읽는다
argo -> k8s: 차이가 있으면 적용
k8s -> argo: 실제 상태 보고 { style.stroke-dash: 3 }
argo -> obs: 드리프트 알림 { style.stroke-dash: 3 }
```
...
회전과 파드 재시작, 복구 순서 — 는 **[10장](/onprem/10-secrets/)** 에서 통째로 다룬다.
:::
...
| **차트 저장소 이전** | Loki 차트처럼 저장소가 옮겨 가는 일이 있다 ([관측 덱 3장](/observability/03-loki/)) |
| **Argo CD 자신의 알림** | `argocd_app_info{sync_status="OutOfSync"}`를 Prometheus 알림으로 ([관측 덱 2장](/observability/02-prometheus/)) |
...
- 시크릿은 **부트스트랩 순서에서 앱보다 먼저** 온다 — 실무 상세는 [10장](/onprem/10-secrets/)
