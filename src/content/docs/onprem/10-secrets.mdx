...
```mermaid
flowchart LR
    Q1["① Git에는 무엇을 두나"] --> A1["암호문 또는<br/>외부 금고 참조"]
    Q2["② 여러 곳에 어떻게 주나"] --> A2["네임스페이스별 생성<br/>또는 안전한 복제"]
    Q3["③ 원천 값이 바뀌면"] --> A3["Secret 갱신<br/>앱이 다시 읽게 한다"]
    Q4["④ 클러스터를 잃으면"] --> A4["복호화 수단 복구<br/>그다음 Secret 재생성"]
...
```mermaid
flowchart LR
    AGE["age identity · 개인키<br/>(오프라인 금고 · 사람)"] -->|"SOPS로 복호화"| MK["sealing key 세트 백업<br/>(.enc — 암호화된 파일)"]
    MK -->|"클러스터에 주입"| CTL["Sealed Secrets 컨트롤러"]
    SS["SealedSecret 암호문<br/>(Git)"] -->|"복호화 요청"| CTL
    CTL --> SEC["런타임 Secret"]
    SEC --> POD["파드"]
