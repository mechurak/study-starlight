...
<TermIntro
  terms={[
    ...
    ['DR', 'Disaster Recovery, 재해 복구', '사이트·클러스터 전체를 잃었을 때 다른 환경에서 서비스를 되살리는 계획과 절차. **백업 파일만이 아니라 복구 순서와 연습**까지 포함한다.'],
  ]}
/>
...
|---|---|---|
| **매니페스트·설정** | 무엇이 있었는지 모른다 | **Git** ([9장](/onprem/09-gitops/)) |
...
| **Sealed Secrets sealing key 세트** | **Git의 모든 비밀을 영영 못 푼다** | 별도 금고 ([10장](/onprem/10-secrets/)) |
...
```d2 layout=elk pad=40 title="매니페스트·etcd·PV·PostgreSQL·sealing key 다섯 백업 대상이 각각 Git 저장소·오브젝트 스토리지·금고로 가고, 오브젝트 스토리지는 다시 오프사이트로 복제되는 보관 구조"
direction: down
...
src: 백업 대상 {
  grid-rows: 5
  git2: 매니페스트 { class: mute }
  etcd: etcd { class: mute }
  pv: PV 데이터 { class: mute }
  db: PostgreSQL { class: mute }
  key: sealing key 세트 · CA 키 { class: bad }
}
dst: 보관처 {
  grid-rows: 4
  gitr: "Git 저장소\n클러스터 밖" {
...
src.git2 -> dst.gitr
src.etcd -> dst.s3: 스냅샷
src.pv -> dst.s3: Velero
src.db -> dst.s3: 베이스백업 + WAL
dst.s3 -> dst.off: 복제
src.key -> dst.vault
```
...
| sealing key 세트·CA 키 | **변경·갱신 시마다** | 0 | 금고에서 복원 ([10장](/onprem/10-secrets/)) | 분 |
...
3. **Sealed Secrets sealing key 세트**를 복원한다 — 없으면 Git의 비밀을 못 푼다 ([10장](/onprem/10-secrets/))
