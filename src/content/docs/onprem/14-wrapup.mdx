...
```d2 layout=elk pad=40 title="⑤ 서비스부터 ① 바닥까지 다섯 층에 각 장에서 채운 도구를 얹어 정리한 온프렘 플랫폼 전체 지도"
grid-rows: 5
...
l4: ④ 운영 도구 {
  grid-rows: 3
  argo: Argo CD · 시크릿 — 9~10장 { class: mute }
  obs: 관측 스택 — 8장 · 관측 덱 { class: mute }
  vel: Velero · etcd 스냅샷 — 11장 { class: mute }
}
l3: ③ 상태 {
  grid-rows: 2
  s3: 오브젝트 스토리지 — 6장 {
...
l2: ② 입구 {
  grid-rows: 3
  net: MetalLB · Gateway API — 3장 { class: zone }
  tls: cert-manager · external-dns — 4장 { class: zone }
  auth: Keycloak · oauth2-proxy — 5장 { class: zone }
}
l1: ① 바닥 {
  base: 노드 · etcd · CNI · CSI — 2장 { class: zone }
}
