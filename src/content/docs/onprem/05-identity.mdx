...
<TermIntro
  terms={[
    ...
    ['OIDC', 'OpenID Connect', 'OAuth 2.0 위에 "누구인지"(신원)를 얹은 프로토콜. 앱은 ID 토큰을 받아 사용자를 안다. 상세는 [Keycloak OAuth와 OIDC](/keycloak/oauth-oidc/).'],
...
```d2 layout=elk pad=40 title="앱이 OIDC를 지원하면 패턴 ①로 직접 Keycloak과 대화하고, 아니면 쿠버네티스 API 서버인지에 따라 패턴 ③ kubelogin과 패턴 ② oauth2-proxy로 갈리는 분기도"
direction: down
...
p1: "패턴 ①\n앱이 직접 Keycloak과 대화\n(Grafana · Argo CD · MinIO)" { class: ok }
p3: "패턴 ③\nAPI 서버 OIDC + kubelogin" { class: warn }
p2: "패턴 ②\n앞에 oauth2-proxy를 세운다\n(Prometheus · Alertmanager · 사내 대시보드)" { class: warn }
...
| **Kubernetes API** | ③ | → [Keycloak Kubernetes OIDC](/keycloak/kubernetes-oidc/) |
...
```d2 layout=elk pad=40 title="Gateway가 요청을 oauth2-proxy에 물어 미인증이면 Keycloak 로그인으로 보내고, 인증되면 사용자 헤더와 함께 Prometheus로 전달하는 흐름"
direction: down
...
u: 사용자 { class: mute }
gw: Gateway { class: mute }
op: oauth2-proxy { class: warn }
kc: Keycloak { class: warn }
app: Prometheus { class: mute }
...
```d2 layout=elk pad=40 title="AD 그룹이 LDAP 매퍼로 Keycloak 그룹이 되고 groups claim으로 ID 토큰에 실린 뒤 도구별 매핑으로 Grafana Admin·Argo CD role:admin이 되는 권한 사슬"
direction: down
...
ad: "AD 그룹\nCN=platform-admins" { class: mute }
kg: Keycloak 그룹 { class: mute }
tok: ID 토큰 { class: key }
role: "Grafana Admin\nArgo CD role:admin" { class: mute }
...
2. **Keycloak에서 groups claim을 토큰에 싣는다.** client scope와 mapper 설정이며,
   [Keycloak Client와 SSO](/keycloak/clients-and-sso/)가 다룬다.
...
4. **claim이 실제로 도착하는지 확인한다.** 안 되면 사슬 어디가 끊겼는지 순서대로 본다 —
   ...
   [Keycloak Scope와 Mapper](/keycloak/scopes-and-mappers/)와 [문제 진단](/keycloak/troubleshooting/)에서 확인한다.
...
| 문제 | 왜 생기나 | 어떻게 다루나 |
|---|---|---|
| **로그아웃이 한 번에 안 된다** | 세션이 세 층(앱 쿠키 · 프록시 쿠키 · Keycloak SSO 세션) | 도구 로그아웃 링크를 Keycloak의 end-session으로 보낸다 ([Keycloak Session과 Logout](/keycloak/sessions-and-logout/)) |
| **Keycloak이 죽으면 아무도 못 들어온다** | 신원을 한 곳에 모은 대가 | Keycloak의 [저장소와 가용성](/keycloak/storage-and-availability/)을 설계하고 **깨진 유리 계정**을 도구마다 하나 남긴다 |
| **자동화는 사람 로그인을 못 한다** | CI(Continuous Integration, 지속적 통합)·스크립트에는 브라우저가 없다 | 서비스 계정 토큰(Argo CD)·API 키를 따로 두고 **감사 로그를 남긴다** |
