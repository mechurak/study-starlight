...
flowchart LR
    ...
    subgraph L4["④ 운영 도구"]
        direction TB
        ARGO["Argo CD · 시크릿 — 13장"]
        OBS["Prometheus · Loki · Tempo · Grafana — 8~12장"]
        VEL["Velero · etcd 스냅샷 — 14장"]
    end
    subgraph L3["③ 상태"]
        direction TB
        S3[("오브젝트 스토리지 — 6장")]
...
| [13](/onprem/13-gitops/) | "뭐가 깔려 있죠?"에 **Git 주소로** 답한다. **마스터키 백업이 곧 복구 가능성** |
| [14](/onprem/14-backup/) | GitOps는 **절반만** 백업한다. **복구는 리허설한 것만** 된다 |
| [15](/onprem/15-ops/) | 운영의 8할은 **디스크 · 인증서 · 버전**. 진단은 **층을 위에서 아래로** |
...
8. **배포** ([13장](/onprem/13-gitops/))
   ...
   - 시크릿 체계 + **마스터키를 클러스터 밖 금고에**
   - 사내 레지스트리 미러, 태그 고정

9. **백업과 리허설** ([14장](/onprem/14-backup/))
   - etcd 스냅샷 cron + 노드 밖 전송
   - Velero 스케줄, **오프사이트 복제**
   - **분기 리허설 일정을 달력에 등록** — 여기까지 해야 끝이다

10. **운영 루틴** ([15장](/onprem/15-ops/))
    - 일·주·월·분기 항목 정하기
    - 알림마다 runbook 링크
...
4. 되돌려야 하면 **새 Cluster로 PITR** — 원본은 건드리지 않는다 ([14장](/onprem/14-backup/))
</Card>
<Card title="로그·메트릭이 안 보인다" icon="warning">
1. **관측 스택 자체가 살아 있나** — 그래프가 평평한 것과 정상은 다르다 ([15장](/onprem/15-ops/))
