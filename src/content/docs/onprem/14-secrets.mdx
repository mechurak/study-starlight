...
| 문제 | 왜 |
|---|---|
| **Git에 못 올린다** | GitOps는 Git이 단일 소스인데([13장](/onprem/13-gitops/)), 평문 YAML을 올리면 저장소를 읽는 모두가 본다 |
| **etcd에 평문으로 남는다** | 저장 시 암호화를 안 켰다면 etcd 스냅샷에 원문이 들어 있다 ([15장](/onprem/15-backup/)) |
...
kubectl -n sealed-secrets get secret \
  -l sealedsecrets.net/sealed-secrets-key -o yaml > sealed-secrets-master.key
```
:::
...
[15장](/onprem/15-backup/)의 백업 목록에서 이 키가 1순위인 이유다.
:::
...
| 사내 레지스트리 pull 시크릿 | [13장](/onprem/13-gitops/) — **모든** 네임스페이스에 필요 |
...
| **etcd 스냅샷** | 저장 시 암호화(EncryptionConfiguration)를 켜고, 스냅샷 파일 접근을 Secret 수준으로 통제 ([15장](/onprem/15-backup/)) |
...
[15장](/onprem/15-backup/)의 클러스터 복구 절차 안에서 **시크릿은 아주 이른 단계**에 온다.
...
kubectl -n sealed-secrets get secret -l sealedsecrets.net/sealed-secrets-key \
