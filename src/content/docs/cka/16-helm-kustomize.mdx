...
```mermaid
flowchart LR
    U["helm upgrade"] --> Q{"값 처리 방식을<br/>어떻게 정했는가"}
    Q -->|"-f 또는 --set"| NEW["이번에 준 값을 적용한다"]
    Q -->|"--reuse-values"| KEEP["이전 사용자 값에<br/>이번 값을 병합한다"]
    Q -->|"--reset-values"| RESET["대상 차트 기본값에서<br/>다시 시작한다 ⚠️"]
