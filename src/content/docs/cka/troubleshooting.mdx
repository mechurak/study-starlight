...
클러스터는 결국 **선언된 상태(spec)를 실제 상태(status)로 맞추는 루프들의 모음**이다 ([시작하기 전에](/cka/intro/)).
...
| `unbound immediate PersistentVolumeClaims` | PVC 미바인딩 | [스토리지 진단](/cka/storage-troubleshooting/) |
| `didn't match pod anti-affinity rules` | antiAffinity | [스케줄링](/cka/scheduling/) |
...
`Liveness probe failed` 가 반복되고 있을 것이다 ([Pod](/cka/pods/)).
:::
...
노드 메모리 부족 → **kubelet이 Pod을 축출(Evicted)**, QoS(Quality of Service) 등급 순서대로 ([설정과 리소스](/cka/config/))
:::
...
| `Unable to attach or mount volumes` | PV/CSI 문제 ([스토리지 진단](/cka/storage-troubleshooting/)) |
...
볼륨이 detach될 때까지 새 Pod은 계속 `ContainerCreating`에 머문다 ([스토리지 진단](/cka/storage-troubleshooting/)).
...
오브젝트에 finalizer(삭제 전에 끝내야 할 정리 작업을 표시해 둔 문자열 목록 — [CRD](/cka/crd/))가
...
남은 오브젝트가 없음을 확인**해야 하는데, 집계된 API 서버([확장](/cka/extensions/)) 하나가 응답하지 않으면
...
`crictl`과 `journalctl`로 내려간다 ([확장](/cka/extensions/)).
...
| 데이터 디렉터리 권한 오류 | 복구 후 `chown` 누락 ([etcd 백업과 복구](/cka/etcd-backup/)) |
...
[etcd 백업과 복구](/cka/etcd-backup/)의 백업·복구**이고, 여기서는 "API 서버가 이상하면 etcd 로그부터 본다"는 습관이면 된다.
...
| egress가 전부 안 된다 | **NetworkPolicy의 DNS 미허용** | [NetworkPolicy](/cka/networkpolicy/) |
...
:::note[손 연습]
이 장 범위의 KodeKloud 실습 풀이 — [metrics와 로그](/cka-udemy/troubleshooting/),
[컨트롤 플레인](/cka-udemy/control-plane-failure/), [워커 노드](/cka-udemy/worker-failure/),
[Service와 네트워크](/cka-udemy/network-failure/)
:::
