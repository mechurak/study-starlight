...
[아키텍처](/cka/architecture/)의 그림에서 모든 화살표는 API 서버를 향했다 — kubectl은 그중 **사람이 쏘는 화살표**다.
...
API 서버는 아무 요청이나 받지 않는다 — [아키텍처](/cka/architecture/)에서 본 첫 관문(인증)을 통과해야 한다.
...
반대로 `client-certificate`는 서버에게 "나는 admin이다"를 증명한다. 인증서 체계 자체는 [사용자 인증](/cka/user-authentication/).
...
Events 한 줄에 답이 있는 경우가 대부분이다. ([트러블슈팅](/cka/troubleshooting/))
:::

## 리소스 만들기

만드는 것은 결국 **spec을 API 서버에 밀어 넣는 일**이다 ([아키텍처](/cka/architecture/)). 방법은 두 갈래인데,
...
**이 목록을 손에 붙이는 것이 곧 시험 시간이다.** [시험 전략](/cka/exam-strategy/)에 전체 치트시트가 있다.
...
kubectl·kubeadm은 `--flag=value`와 `--flag value`를 둘 다 받는다. 하지만 **값 생략이 허용되는
플래그는 `=`만 안전하다** — `--dry-run client`라고 띄면 `client`가 플래그 값이 아니라 별도
인자로 해석된다(값 없는 `--dry-run`도 유효한 표기라서, 뒤의 단어를 값으로 붙여 주지 않는다).
불리언 플래그(`--dry-run false` 시절 표기)와 값이 `-`로 시작하는 경우도 같은 함정이다.
어느 플래그가 어느 형태였는지 고민하지 않도록 습관을 `=` 하나로 통일한다.

따옴표는 CLI가 아니라 **셸**을 위한 것이다. `--pod-network-cidr=172.17.0.0/16` 같은 값은
점·슬래시가 셸에 평범한 글자라 안 묶어도 되고, 공백이나 `*`·`?`·`$` 같은 셸 특수문자가
들어 있을 때만 묶는다(`apt-get install kubeadm='1.35.0-*'`의 따옴표가 그 경우 — 없으면 셸이
`*`를 파일명 글롭으로 펼치려 든다).
...
리소스(cpu·메모리)는 v1.35부터 **`--subresource=resize`로 제자리 수정**할 수 있다 ([Pod](/cka/pods/)).
...
fin: finalizer 를 의심한다 · 트러블슈팅 { class: bad }
...
반대로 **네임스페이스가 `Terminating`에서 안 끝나면** finalizer(삭제 전 정리가 끝날 때까지 삭제를 막는 표식 — [트러블슈팅](/cka/troubleshooting/))를 의심한다.
:::

## 진단 도구

여기부터는 **루프가 어디서 멈췄는지**를 알아내는 도구다 ([시작하기 전에](/cka/intro/)의 축, 커리큘럼의 30%).
...
:::note[손 연습]
이 장 범위의 KodeKloud 실습 풀이 — [CKA 실습 1장 · kubectl 기본 조작](/cka-udemy/01-basics/)과
[2장 · JSONPath 추출과 정렬](/cka-udemy/jsonpath/)
:::
