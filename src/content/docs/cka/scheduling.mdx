...
- 축출은 **PDB**(PodDisruptionBudget — "동시에 죽어도 되는 Pod 수"의 하한 선언, [워크로드](/cka/workloads/))를 존중한다 — 위반하면 기다린다

:::tip[시험]
노드 업그레이드 문제의 정해진 순서다.
**`drain` → 작업 → `uncordon`.**
`--ignore-daemonsets`를 빼면 **거의 항상 에러가 난다.** 습관으로 붙일 것. ([노드 유지보수](/cka/node-maintenance/))
:::
...
admission 파이프라인 전체는 [아키텍처](/cka/architecture/), 컨트롤러를 켜고 끄는 방법은 [Admission](/cka/admission/)이다.
...
| `pod has unbound immediate PersistentVolumeClaims` | PVC가 Bound 안 됨 ([PV/PVC 바인딩](/cka/persistent-volumes/#바인딩-규칙)) |
...
:::note[손 연습]
이 장 범위의 KodeKloud 실습 풀이 — [CKA 실습 Pod 배치 조건](/cka-udemy/scheduling/)
:::
