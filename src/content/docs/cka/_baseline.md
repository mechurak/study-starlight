...
- 덱 전체의 축은 **"선언된 상태(spec)와 실제 상태(status)의 차이를 줄이는 루프"** — `intro`와 `wrapup`이 이걸 감싼다
- 각 장이 **"실제로 무엇이 일어나는가" → 명령 → 함정** 순서로 간다
- 배점 순서와 학습 순서를 구분한다 — 트러블슈팅(30%)이 트러블슈팅을 뒤에 두는 건 앞의 전부가 재료라서다
- 장 끝마다 요약 절. `exam-strategy`는 치트시트, `wrapup`은 개념별 한 줄 요약 — **시험 직전에 이 둘만 봐도 되게** 유지한다


## 페이지 구성

- CKA 덱은 제목·파일명·URL에 페이지 번호를 붙이지 않는다. 순서는 `_deck.mjs`의 `sidebar` 배열에서 관리한다.
