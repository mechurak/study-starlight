...
그래서 Pod을 직접 만들지 않고 Deployment 같은 컨트롤러에 맡긴다 ([워크로드](/cka/workloads/)).

### Pod 안에서 공유되는 것

```d2 layout=elk pad=40 title="Pod 안의 app·sidecar 컨테이너가 pause 컨테이너의 네트워크와 볼륨은 공유하지만 파일시스템은 각자 별개라는 구조"
direction: down

classes: {
  key: { style: { fill: "#dbeafe"; stroke: "#2563eb"; font-color: "#1e3a8a" } }
  sh: { style: { fill: "#dcfce7"; stroke: "#16a34a"; font-color: "#14532d" } }
  no: { style: { fill: "#fee2e2"; stroke: "#dc2626"; font-color: "#7f1d1d" } }
}

pod: Pod — 10.244.1.5 {
  pause: "pause 컨테이너\n네트워크 네임스페이스 소유" { class: key }
  c1: "컨테이너 app\n:8080"
  c2: "컨테이너 sidecar\n:9090"
  vol: "volumes\nemptyDir · configMap …" { shape: cylinder; class: sh }
}

fs1: "app 의 파일시스템\n별개 ❌" { class: no }
fs2: "sidecar 의 파일시스템\n별개 ❌" { class: no }

pod.c1 -> pod.c2: localhost:9090
pod.c1 -> pod.vol
pod.c2 -> pod.vol
pod.c1 -> pod.pause: 합류 { style.stroke-dash: 3 }
pod.c2 -> pod.pause: 합류 { style.stroke-dash: 3 }
fs1 -- pod.c1: { style.stroke-dash: 3 }
fs2 -- pod.c2: { style.stroke-dash: 3 }
```
...
```d2 pad=40 title="kubelet 이 pause 컨테이너를 먼저 만들어 Pod IP 를 쥐게 하고, app 컨테이너가 크래시 후 재시작해도 같은 네임스페이스에 합류해 IP 가 유지되는 순서"
...
pa.n1: Pod IP 10.244.1.5 를 쥐고 있다
kl -> ap: 생성 — pause 의 네임스페이스에 합류
ap -> ap: 크래시 { style.stroke-dash: 3 }
kl -> ap: 재시작 후 같은 네임스페이스에 다시 합류
pa.n2: Pod IP 는 그대로 · pause 가 살아 있으므로
```
...
노드에서 `crictl ps`에 앱 컨테이너만 보이는 이유를 이해하는 배경 지식으로 이만큼이면 충분하다.

## 매니페스트 작성

이 YAML이 [시작하기 전에](/cka/intro/)에서 말한 "**선언된 상태**"(spec) 그 자체다. `kubectl apply`로 API 서버에
...
세 이름 중 커리큘럼에 올라 있는 것은 사이드카뿐이다 (아래 "네이티브 사이드카").
앰배서더·어댑터는 분류용 이름이라 깊게 팔 필요는 없다 — 이름만 알아두자.

**함께 넣을지는 세 질문으로 판단한다.**

```d2 layout=elk pad=40 title="같은 노드·같은 볼륨과 localhost·같은 수명 세 조건을 모두 만족할 때만 한 Pod 에 넣고, 하나라도 아니면 별도 Pod 와 Service 로 나눈다"
direction: down
...
q1: "같은 노드에\n있어야만 하는가" { shape: diamond; class: mute }
q2: "같은 볼륨 · localhost 를\n써야 하는가" { shape: diamond; class: mute }
q3: 수명이 같아야 하는가 { shape: diamond; class: mute }
sep: 별도 Pod + Service 로 연결 ✅ { class: ok }
same: "같은 Pod 에 넣는다\n함께 스케일되고 함께 죽는다" { class: key }
...
```d2 layout=elk pad=40 title="init 컨테이너가 순서대로 exit 0 으로 끝나야 containers 가 시작되고, 실패하면 restartPolicy 에 따라 재시도한다"
direction: down
...
s: Pod 스케줄됨 { class: mute }
i1: "init 1\n실행 → exit 0" { class: mute }
i2: "init 2\n실행 → exit 0" { class: mute }
c: containers 시작 { class: ok }
r: "restartPolicy 에 따라 재시도\nInit:CrashLoopBackOff" { class: bad }
...
```d2 layout=elk pad=40 title="네이티브 사이드카는 init 자리에서 먼저 시작해 app 보다 나중에 끝나므로 Job 도 정상적으로 완료된다"
direction: down

classes: {
  side: { style: { fill: "#fef3c7"; stroke: "#d97706"; font-color: "#78350f" } }
  ok: { style: { fill: "#dcfce7"; stroke: "#16a34a"; font-color: "#14532d" } }
  mute: { style: { fill: "#f1f5f9"; stroke: "#94a3b8"; font-color: "#334155" } }
}

st: Pod 시작 { class: mute }
sc: "사이드카 시작\ninit 자리 · restartPolicy Always" { class: side }
ap: app 컨테이너 시작 { class: mute }
run: 둘 다 동작 { class: mute }
ape: app 종료 { class: mute }
sce: "사이드카 종료\n★ 나중에 끝난다" { class: side }
done: Job 이라면 여기서 완료 ✅ { class: ok }
...
"`initContainers`에 넣고 `restartPolicy: Always`를 준다"는 조합 자체를 기억해 두자.
시험 중 init 컨테이너·사이드카 YAML 뼈대가 필요하면 공식 [Init Containers 문서](https://kubernetes.io/docs/concepts/workloads/pods/init-containers/)의 예시를 복사해 이름·이미지·command만 바꾸는 게 가장 빠르다.

## 상태와 재시작

매니페스트가 "선언된 상태"라면 이 절은 전부 반대쪽, "**실제 상태**"(status) 이야기다 —
[시작하기 전에](/cka/intro/)의 루프에서 차이를 판정할 때 읽는 재료가 여기 모여 있다.
문제가 생기면 결국 이 status를 읽는 것부터 시작한다.
...
```d2 layout=elk pad=40 title="Pod 이 Pending 에서 Running 으로 가고 Succeeded · Failed 로 끝나거나 통신 두절 시 Unknown 을 오가는 상태 전이"
direction: down
...
note: "아직 스케줄되지 않았거나\n이미지를 받는 중" { style: { fill: "#f1f5f9"; stroke: "#94a3b8"; font-color: "#334155" } }
...
succeeded -> done
failed -> done
note -- pending: { style.stroke-dash: 3 }
```
...
```d2 layout=elk pad=40 title="컨테이너 state 가 Waiting · Running · Terminated 중 무엇이냐에 따라 어느 명령으로 원인을 찾는지"
direction: right
...
st: 컨테이너 state { shape: diamond }
w1: ContainerCreating → Events { class: warn }
w2: "ImagePullBackOff ·\nErrImagePull → Events" { class: warn }
w3: "CrashLoopBackOff →\n★ logs --previous" { class: bad }
w4: "CreateContainerConfigError\n→ Events" { class: warn }
r: 정상 ✅ { class: ok }
t1: "OOMKilled →\ndescribe 의 Last State" { class: bad }
t2: Error → logs { class: bad }
...
```d2 layout=elk pad=40 title="크래시가 반복되면 재시작 간격이 10·20·40초로 두 배씩 늘어 최대 5분에서 멈추고, 10분간 정상이면 카운터가 초기화된다"
direction: down
...
c: 컨테이너 크래시 { class: mute }
r1: 10초 후 재시작 { class: mute }
r2: 20초 { class: mute }
r3: 40초 { class: mute }
r4: 지수적으로 증가 { class: mute }
max: 최대 5분에서 멈춤 { class: bad }
reset: 카운터 초기화 ✅ { class: ok }
...
```d2 layout=elk pad=40 title="kubelet 이 돌리는 세 프로브 — liveness 실패는 재시작, readiness 실패는 엔드포인트 제외, startup 은 성공할 때까지 나머지 둘을 멈춘다"
direction: down
...
lp: "livenessProbe\n살아 있나?" { class: mute }
rp: "readinessProbe\n요청 받을 준비 됐나?" { class: mute }
sp: "startupProbe\n기동이 끝났나?" { class: mute }
a1: "컨테이너 재시작 ❌\n행에 걸린 앱을 구제" { class: bad }
a2: "Service 엔드포인트에서 제외 ⚠️\n재시작하지 않는다" { class: warn }
a3: "다른 두 프로브를 멈춘다\n느리게 뜨는 앱용" { class: key }
...
시험에 주로 나오는 건 `httpGet`과 `exec`다 — `grpc:`는 이런 게 있다는 것만 알아두면 된다.
...
```d2 layout=elk pad=40 title="initialDelaySeconds 뒤 첫 검사가 돌고 periodSeconds 마다 반복해 failureThreshold 에 도달하면 실패로 판정한다"
direction: down
...
s: 컨테이너 시작 { class: mute }
p1: 1차 검사 { class: mute }
p2: 2차 검사 { class: mute }
p3: 3차 검사 { class: mute }
act: 실패로 판정 → 재시작 { class: bad }

s -> p1: "initialDelaySeconds\n기본 0"
p1 -> p2: "periodSeconds\n기본 10"
p2 -> p3: 10초
p3 -> act: failureThreshold 3 도달
```
...
```d2 layout=elk pad=40 title="Pod 삭제 시 엔드포인트 제거와 preStop 훅이 돌고 SIGTERM 뒤 유예 시간을 기다렸다가 초과하면 SIGKILL 로 강제 종료된다"
direction: down
...
d: 삭제 요청 { class: mute }
p1: "1. Endpoint 에서 제거\n새 트래픽 차단" { class: key }
p2: 2. preStop 훅 실행 { class: key }
s: 3. 컨테이너에 SIGTERM { class: mute }
w: "terminationGracePeriodSeconds\n대기 · 기본 30초" { class: mute }
ok: 정상 종료 ✅ { class: ok }
k: 4. SIGKILL — 강제 종료 ❌ { class: bad }
...
**1번과 2번은 동시에 시작한다.** 그래서 엔드포인트 제거(Service의 대상 목록에서 빼는 것 — [Service](/cka/services/))가
전파되기 전에 SIGTERM이 도착할 수 있다 — `preStop`에 `sleep 5` 를 넣는 관행이 여기서 나온다.

### lifecycle 훅<Badge text="시험 비중 낮음" variant="note" />
...
훅 자체를 깊게 묻는 일은 드물다 — 위 종료 흐름에서 `preStop`에 `sleep`을 넣는
...
`fieldRef`·`resourceFieldRef`로 값을 끌어오는 통로가 **Downward API**다 —
...
`envFrom:` 을 쓰면 ConfigMap/Secret의 **모든 키를 한 번에** 환경변수로 넣을 수 있다 ([설정과 리소스](/cka/config/)).
...
```d2 layout=elk pad=40 title="Pod 와 컨테이너의 securityContext 가 겹치면 컨테이너 쪽이 이기고, fsGroup 은 Pod 수준에만 capabilities 는 컨테이너 수준에만 있다"
direction: down
...
p: "Pod securityContext\nrunAsUser · runAsGroup · fsGroup" { class: mute }
c: "컨테이너 securityContext\nrunAsNonRoot · capabilities …" { class: key }
w: 최종 적용 값 { class: mute }
only1: fsGroup 은 Pod 수준에만 { class: warn }
only2: capabilities 는 컨테이너 수준에만 { class: warn }

p -> c
c -> w: 겹치면 컨테이너가 이긴다
only1 -- p: { style.stroke-dash: 3 }
only2 -- c: { style.stroke-dash: 3 }
```
...
필드 이름이 기억나지 않으면 공식 [Security Context 문서](https://kubernetes.io/docs/tasks/configure-pod-container/security-context/)의 YAML을 복사해 값만 바꾸면 된다.
:::
...
컨트롤 플레인 컴포넌트 자신이 이 방식으로 뜬다 ([아키텍처](/cka/architecture/)).
...
`resources`는 원래 만들고 나면 못 바꾸는 필드였다 — 조정하려면 Pod을 지우고
...
- **QoS 클래스는 바뀔 수 없다** ([설정과 리소스](/cka/config/))
- `status.containerStatuses[*].resources` 에 **실제 적용된 값**이 보인다

v1.35에서 GA가 되며 커리큘럼에 올라온 항목이다 —
`kubectl patch`에 `--subresource=resize`를 붙이는 것이 핵심이다.
...
:::note[손 연습]
이 장 범위의 KodeKloud 실습 풀이 — [CKA 실습 3장 · Pod 생성과 상태 확인](/cka-udemy/pods/),
[6장 · command와 args](/cka-udemy/03-pod-config/), [8장 · init과 sidecar](/cka-udemy/init-sidecar/)
:::
