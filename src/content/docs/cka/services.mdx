...
답부터 말하면 **노드마다 도는 kube-proxy가 커널에 심어 둔 규칙**이다 ([아키텍처](/cka/architecture/)의 노드 컴포넌트).
[시작하기 전에](/cka/intro/)의 축이 여기에도 그대로 적용된다 — Service·EndpointSlice라는 **선언**을 kube-proxy가
...
누가 실행하는지는 [확장 CNI 절](/cka/extensions/#cni--pod-네트워크)에서 본다.
...
- 엔드포인트 컨트롤러(kube-controller-manager 안의 제어 루프 중 하나 — [아키텍처](/cka/architecture/))가
...
- StatefulSet의 Pod별 안정적 DNS ([워크로드](/cka/workloads/))
...
q3: "이름으로\n되는가" { shape: diamond; class: mute }
a3: "DNS 문제 ·\nCoreDNS · DNS" { class: bad }
q4: "다른 네임스페이스\n· 외부에서는" { shape: diamond; class: mute }
a4: "FQDN 필요 또는\nNetworkPolicy · NetworkPolicy" { class: bad }
...
| ClusterIP는 되는데 이름이 안 된다 | **DNS 문제** — CoreDNS를 본다 ([DNS](/cka/dns/)) |
| 다른 네임스페이스에서만 안 된다 | **FQDN(Fully Qualified Domain Name — 네임스페이스까지 붙인 완전한 이름, [DNS](/cka/dns/)) 필요** 또는 **NetworkPolicy** ([NetworkPolicy](/cka/networkpolicy/)) |
...
:::note[손 연습]
이 장 범위의 KodeKloud 실습 풀이 — [CKA 실습 15장 · Service와 EndpointSlice](/cka-udemy/05-services-dns/)
:::
