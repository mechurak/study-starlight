...
단일 인스턴스 DB라면 Deployment + PVC(PersistentVolumeClaim — 저장소 요청, [Pod별 저장소와 업데이트](/cka/statefulset-storage/))로도 충분하다.
:::
...
버전 공존이 불가능할 때** 필요하다 ([Pod별 저장소와 업데이트](/cka/statefulset-storage/)).
...
- HPA(Horizontal Pod Autoscaler — 부하에 따라 replicas를 자동 조절)가 붙어 있으면
  **수동 스케일은 곧 되돌려진다** ([오토스케일링](/cka/autoscaling/))
...
- **taint가 걸린 노드에는 안 뜬다** — `tolerations`가 필요하다 ([스케줄링](/cka/scheduling/))
...
노드 업그레이드로 `kubectl drain`을 돌릴 때([노드 유지보수](/cka/node-maintenance/)) 한 앱의 Pod들이 한꺼번에 축출되면
...
`kubectl get pdb`의 `ALLOWED DISRUPTIONS`가 0이면 이 상황이다. ([노드 유지보수](/cka/node-maintenance/))
:::
...
:::note[손 연습]
이 장 범위의 KodeKloud 실습 풀이 — [CKA 실습 Deployment rollout과 rollback](/cka-udemy/workloads/)과
[Job 완료와 실패](/cka-udemy/jobs/)
:::
