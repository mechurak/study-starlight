...
```mermaid
flowchart LR
    T["Troubleshooting<br/>30%<br/>18장"]
    C["Cluster Architecture<br/>25%<br/>14~17장"]
    N["Servicing and Networking<br/>20%<br/>9~12장"]
    W["Workloads and Scheduling<br/>15%<br/>4~8장"]
    S["Storage<br/>10%<br/>13장"]
    T --- C
    C --- N
    N --- W
    W --- S
    SUM["T + C = 55%<br/>여기가 승부처다"] -.- T
    SUM -.- C

    classDef big fill:#fee2e2,stroke:#dc2626,color:#7f1d1d
    classDef mid fill:#fef3c7,stroke:#d97706,color:#78350f
    classDef small fill:#dbeafe,stroke:#2563eb,color:#1e3a8a
...
flowchart LR
    NEW["최근 개정에서 강조된 것"]
    NEW --> G["Gateway API<br/>Ingress 만 알면 안 된다 · 11장"]
    NEW --> H["Helm / Kustomize<br/>설치·값 오버라이드 · 16장"]
    NEW --> C["CRD와 오퍼레이터<br/>설치하고 상태 확인 · 17장"]
    NEW --> A["워크로드 오토스케일링<br/>HPA + metrics-server · 8장"]
    NEW --> L["컨테이너 출력 스트림<br/>로그 전반 · 18장"]
    OLD["오래된 자료의 함정"] -.->|"Docker · PodSecurityPolicy 를 다루고<br/>Gateway API 가 없다 ❌"| NEW
...
```mermaid
flowchart LR
    Q1["문제 N"] --> SSH["ssh node01"] --> WORK["노드에서 작업"]
    WORK -->|"exit 했다 ✅"| BACK["원래 셸<br/>다음 문제 정상"]
    WORK -->|"exit 을 잊었다 ❌"| STUCK["노드 셸에 남는다<br/>kubectl 이 다른 kubeconfig 를 보거나 아예 없다"]
    STUCK --> WHY["'갑자기 명령이 안 먹는다' 의 90%"]
...
```mermaid
flowchart LR
    OK["열 수 있다 ✅"] --> O1["kubernetes.io/docs/<br/>검색 기능 포함"]
    OK --> O2["kubernetes.io/blog/"]
    OK --> O3["helm.sh/docs/"]
    OK --> O4["gateway-api.sigs.k8s.io/"]
    OK --> O5["문제의 Quick Reference 링크"]
    NG["열면 안 된다 ❌"] --> N1["GitHub"]
    NG --> N2["StackOverflow"]
    NG --> N3["블로그 전부"]
    NOTE["검색은 되지만<br/>외부 사이트 클릭에 주의"] -.- NG

    classDef ok fill:#dcfce7,stroke:#16a34a,color:#14532d
    classDef bad fill:#fee2e2,stroke:#dc2626,color:#7f1d1d
    classDef mute fill:#f1f5f9,stroke:#94a3b8,color:#334155
    class OK,O1,O2,O3,O4,O5 ok
    class NG,N1,N2,N3 bad
    class NOTE mute
```
...
```mermaid
flowchart LR
    M1["kubectl create 로 만들었다"] --> R["클러스터의 최종 상태"]
    M2["YAML 파일로 만들었다"] --> R
    M3["kubectl edit 으로 고쳤다"] --> R
    R --> SCORE["자동 검증 스크립트가 채점<br/>어떻게 만들었는지는 상관없다"]
    SCORE --> PART["부분 점수가 있다<br/>맞춘 요구만큼 받는다"]
