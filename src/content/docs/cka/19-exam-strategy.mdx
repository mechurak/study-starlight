...
- [기본 StorageClass](/cka-udemy/07-storage/#local-sc--기본-클래스와-확장-허용까지-설정): annotation의 `"true"`와 확장 필드의 `true` 구분
- [네이티브 로그 사이드카](/cka-udemy/03-pod-config/#logging-deployment--네이티브-로그-사이드카): `initContainers` + 컨테이너 수준 `restartPolicy: Always`, `touch` 후 `tail -f`
