...
Pod은 서로를 IP가 아니라 `web-svc` 같은 **이름**으로 부른다 ([Service](/cka/services/)).
...
- 레코드는 어디서 얻나 — CoreDNS도 다른 컴포넌트처럼 **kube-apiserver를 watch한다**
  ([시작하기 전에](/cka/intro/)의 루프). Service가 생기면 DNS 레코드가 따라 생기고, 별도의 등록 절차가 없는 이유다
...
headless Service([Service](/cka/services/))에서만 나오는 이름이다. Pod 이름이 `db-0`, `db-1`처럼
고정되는 StatefulSet([워크로드](/cka/workloads/))과 짝을 이뤄, **특정 인스턴스를 이름으로 지목**할 때 쓴다.
...
**Pod을 만들 때 kubelet이 써 넣는 resolv.conf가** 정한다 ([아키텍처](/cka/architecture/)의 노드 컴포넌트).
...
잘못된 것인지 구분되지 않는다 ([Service](/cka/services/)의 진단 표와 같은 문제다). 그래서
...
다만 `nslookup`은 이름 해석만 확인한다. Pod IP나 Service 포트의 실제 reachability는
`wget`, `curl`, `nc`처럼 대상 포트에 연결하는 명령으로 따로 증명한다.
:::
...
e: "Service · 엔드포인트 문제 · Service\n또는 NetworkPolicy · NetworkPolicy" { class: warn }
...
| DNS는 되는데 연결이 안 된다 | Service/엔드포인트 문제 ([Service](/cka/services/)) 또는 **NetworkPolicy** ([NetworkPolicy](/cka/networkpolicy/)) |
| 간헐적으로 느리다 | `ndots:5`로 인한 불필요한 조회, CoreDNS 레플리카 부족 |

:::caution[함정]
**NetworkPolicy로 egress를 막으면 DNS도 막힌다.**
UDP 53번을 명시적으로 허용하지 않으면 이름 해석부터 실패한다. [NetworkPolicy](/cka/networkpolicy/)에서 다룬다.
:::
...
:::note[손 연습]
이 장 범위의 KodeKloud 실습 풀이 — [CKA 실습 16장 · DNS와 CoreDNS](/cka-udemy/dns/)
:::
