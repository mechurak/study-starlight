...
2. **vim 설정** — 탭 문자로 YAML을 깨뜨리지 않기 위해
   ...
   ```bash
   # 기존 설정이 있으면 vim ~/.vimrc로 필요한 항목만 수정
   cat <<'EOF' >> ~/.vimrc
   set expandtab
   set tabstop=2
   set softtabstop=2
...
`:setlocal expandtab tabstop=2 softtabstop=2 shiftwidth=2 autoindent number`를 실행한다.
:::
...
[실습 1장 생성 흐름](/cka-udemy/01-basics/#도움말--yaml-뼈대--필요한-필드만-수정)에서 연습한다.
...
- [Ingress 생성](/cka-udemy/06-ingress-netpol/#webapp-ingress--hostprefixingressclass를-한-명령으로): `--class` → `ingressClassName`, 경로 끝 `*` → `Prefix`, 마지막에는 `curl`
- [apply 롤링 업데이트](/cka-udemy/02-workloads/#nginx-deploy--apply로-생성과-rolling-update를-모두-수행): 파일 수정 후 두 번째 `apply`, 새 이미지·Ready 검증
