...
갈린 것은 되느냐가 아니라 **얼마를 내느냐**였다. 덱 23개에서 Docusaurus는 문서 묶음마다 설정에
인스턴스를 세우지만, 이 레포는 manifest 배열에 항목 하나를 더하면 사이드바와 랜딩 카드가 같이
나온다. 그리고 네이티브로 얻는 대가로 React 런타임이 전 페이지에 실린다 — 바로 다음 절의
zero-JS 이야기가 추상론이 아니라 실제 비용이 되는 지점이다.
...
| **무관한 주제 여러 개를 담는 개인·소규모 학습 노트** | **Starlight** | 내장 기본값 + zero-JS + manifest 한 벌에서 덱마다 사이드바 |
...
- **덱 여러 개**는 출발 조건이었지만 후보 전부가 해내므로 결정 근거가 아니다 — 갈린 것은 그 비용이다 (manifest 한 줄 대 인스턴스마다 설정 + React 런타임)
