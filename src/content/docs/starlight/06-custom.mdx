...
예외는 컴포넌트가 직접 렌더하지 않은 런타임 DOM을 꾸미는 경우다. `MermaidZoom`은
astro-mermaid가 나중에 만든 SVG를 대상으로 하므로 `<style is:global>`이 필요하지만,
선택자를 `.mermaid-frame` · `.mermaid-zoom` 아래로 제한해 영향 범위는 컴포넌트 안에 가둔다.
...
override 때문에 자기 셸을 설치하지 못하므로 `<ImageZoom />`을 직접 합성하고, 클라이언트에서
생기는 Mermaid SVG에는 `<MermaidZoom />`이 카드와 클릭 확대를 덧붙인다. 같은 override를
