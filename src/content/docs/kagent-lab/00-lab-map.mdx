...
```d2 layout=elk title="dashboard·CLI·A2A 세 입구가 같은 kagent controller를 거쳐 Kubernetes API의 Agent·ModelConfig·MCPServer를 읽고, Agent workload가 model provider와 MCP tool server에 닿는 구조"
direction: right
...
입구가 교체 가능하다는 뜻이기도 하다. 사내에 자체 포털을 두면 dashboard 자리를 우리 frontend·backend가
대신하고 dashboard는 운영자 콘솔로 남는다. 그 연결 설계는
...
| `SandboxAgent` | 실행하지 않음. idle actor를 snapshot하고 복원하는 구조는 [9장](/kagent-lab/09-harness-substrate/)에서 개념으로 본다 |
| `AgentHarness` | 실행하지 않음. 장기 coding sandbox도 [9장](/kagent-lab/09-harness-substrate/)에서 개념으로 본다 |
