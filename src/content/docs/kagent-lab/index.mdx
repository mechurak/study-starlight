---
title: 개요
description: 전용 kind 클러스터에서 kagent 설치부터 Declarative Agent·MCP·A2A·debug·BYO·cleanup까지 따라 하는 실습
---
...
flowchart LR
    ...
    INSTALL --> AGENT["Declarative Agent<br/>model · prompt"]
...
<DeckMap
  steps={[
    ...
    { label: '2~3장', href: '/kagent-lab/02-first-agent/', title: '첫 호출과 Declarative Agent', tone: 'ok',
    ...
    { label: '7~8장', href: '/kagent-lab/07-byo-agent/', title: 'BYO 경계와 정리', tone: 'bad',
      ...
      note: 'Declarative 설정을 넘어갈 때 책임이 어떻게 달라지고 무엇을 지워야 하나' },
    { label: '9장', href: '/kagent-lab/09-harness-substrate/', title: 'Agent Harness와 Agent Substrate', tone: 'mute',
      desc: 'SandboxAgent · AgentHarness · worker pool과 snapshot — cluster 없이 읽는 개념 장',
      note: '상주 Deployment로 부족해지는 순간 인프라는 무엇을 준비해야 하나' },
  ]}
/>
...
- Declarative Agent와 BYO Agent의 책임 경계를 설명하고 전용 cluster를 삭제한다.
