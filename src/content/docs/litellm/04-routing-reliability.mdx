...
```d2 layout=elk pad=40 title="chat-general 요청이 deployment 선택과 호출을 거쳐 성공하면 응답으로, 일시 오류면 제한된 retry로 가고 반복 실패는 cooldown, group 실패는 다른 model group fallback으로 이어지는 흐름"
direction: down
...
req: chat-general 요청 { class: key }
pick: "① 같은 group에서\ndeployment 선택" { class: key }
call: ② 호출 { class: warn }
retry: ③ 제한된 retry { class: warn }
cool: ④ deployment cooldown { class: bad }
fall: ⑤ 다른 model group fallback { class: bad }
ok: 응답 { class: ok }
