...
```d2 layout=elk pad=40 title="애플리케이션이 Gateway API를 거쳐 LiteLLM의 인증·접근·예산 계층과 Router를 지나 외부 provider와 사내 모델로 나가고, Postgres·Redis에 상태를 두며 Prometheus와 Langfuse로 관측 신호를 보내는 덱 전체 요약도"
direction: down
...
app: "애플리케이션\nvirtual key · 공개 model" { class: key }
edge: "Gateway API\nDNS · TLS" { class: key }
auth: "LiteLLM\nauth · access · budget · limit" { class: key }
router: "Router\nbalance · retry · cooldown · fallback" { class: zone }
ext: 외부 provider { class: ok }
int: 사내 vLLM / KServe / GPUStack { class: ok }
pg: "Postgres\nkey · team · spend · config" {
  shape: cylinder
  class: warn
}
redis: "Redis\nlimit · router · cache · lock" {
  shape: cylinder
  class: warn
}
prom: Prometheus · 로그 { class: warn }
lf: Langfuse { class: warn }
...
auth -- pg
auth -- redis
auth -> prom: { style.stroke-dash: 3 }
router -> lf: { style.stroke-dash: 3 }
```
