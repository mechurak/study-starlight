...
```d2 layout=elk pad=40 title="사내 애플리케이션이 Gateway API와 Service를 거쳐 온프렘 쿠버네티스의 LiteLLM Pod 두 개로 들어오고, Pod가 ConfigMap·Secret을 공유하며 Pod 밖의 Postgres HA·Redis HA·외부 provider·사내 model endpoint·관측 스택에 연결되는 배치"
direction: down
...
client: 사내 애플리케이션 { class: key }
gw: "Gateway API\nTLS · 내부 DNS" { class: key }
svc: "Service\nlitellm:4000" { class: key }

k8s: 온프렘 Kubernetes {
  p1: "LiteLLM Pod A\n1 worker" { class: zone }
  p2: "LiteLLM Pod B\n1 worker" { class: zone }
  cm: "ConfigMap\nconfig.yaml" { class: zone }
  sec: Secret mount/env { class: zone }
  mig: migration Job { class: zone }
...
ext: 외부 provider { class: ok }
local: 사내 model endpoint { class: ok }
obs: Prometheus · 로그 · Langfuse { class: ok }
...
k8s.p1 -> ext
k8s.p2 -> local
k8s.p1 -> obs: { style.stroke-dash: 3 }
k8s.p2 -> obs: { style.stroke-dash: 3 }
```
