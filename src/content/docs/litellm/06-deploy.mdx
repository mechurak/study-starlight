...
```d2 layout=elk pad=40 title="Git revision이 render와 policy check를 거쳐 migration Job으로 가고, 성공하면 serving rollout과 검증으로, 실패하면 중단하고 DB 복구를 판단하는 배포 흐름"
direction: down
...
git: Git revision { class: key }
render: render · policy check { class: key }
mig: migration Job { class: warn }
roll: serving rollout { class: ok }
stop: 중단 · DB 복구 판단 { class: bad }
verify: synthetic request · telemetry { class: ok }
