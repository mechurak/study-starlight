...
```d2 pad=40 title="애플리케이션 요청이 LiteLLM에서 Redis 캐시 조회, Postgres 정책 조회, 판정, deployment 선택을 거쳐 LLM endpoint로 가고 응답과 함께 spend 기록·callback이 나가는 순서도"
...
key cache miss: {
  ...
  p -> l: 정책 { style.stroke-dash: 3 }
}

l -> l: model access · budget · rate limit 판정
l -> l: deployment 선택 · retry/fallback 계획
l -> m: provider 형식으로 변환한 요청
m -> l: 응답 또는 stream { style.stroke-dash: 3 }
l -> a: 공통 형식 응답 + x-litellm-call-id { style.stroke-dash: 3 }
l -> p: spend 기록 { style.stroke-dash: 3 }
l -> o: success/failure callback { style.stroke-dash: 3 }
```
