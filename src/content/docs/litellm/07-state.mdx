...
```d2 layout=elk pad=40 title="의존성 장애가 Postgres인지 Redis인지로 갈리고, Postgres 쪽은 승인된 fail-open 여부에 따라 fail-closed 또는 degraded serving으로, Redis 쪽은 replica 축소로 이어지는 판단 흐름"
direction: down
...
fail: 의존성 장애 { class: bad }
q: 무엇이 끊겼나 {
  shape: diamond
  class: warn
}
pg: "새 key/cache miss · budget · 관리 기능 영향" { class: bad }
rd: "전역 limit · cooldown · cache 일관성 영향" { class: bad }
safe: "격리된 내부망이며\n승인된 fail-open인가" {
  shape: diamond
  class: warn
}
close: "fail-closed · 명확한 5xx/503" { class: ok }
deg: 제한된 모델만 degraded serving { class: ok }
single: "필요하면 replica/worker를 1로 축소\n한도 의미 보존" { class: ok }
