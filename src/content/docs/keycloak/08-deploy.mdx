...
```d2 layout=elk pad=40 title="ingress가 sticky session으로 keycloak 두 파드에 붙고, 두 파드는 Infinispan으로 서로 묶이며 PostgreSQL을 원본으로 쓰는 HA 구성"
direction: down
...
k8s: k8s 클러스터 {
  i: "ingress\n(sticky session)" { class: mute }
  k1: keycloak-0 { class: key }
  k2: keycloak-1 { class: key }
