...
<TermIntro
  terms={[
    ['Operator', '', '특정 앱의 운영 지식을 코드로 담은 컨트롤러. CR(Custom Resource)에 "이런 Keycloak을 원한다"고 선언하면 StatefulSet·설정을 대신 만든다.'],
    ['Infinispan', '', 'Keycloak에 내장된 분산 캐시. 여러 replica가 세션·무효화 정보를 **서로 공유**하는 통로다.'],
    ['JGroups', '', 'Infinispan이 클러스터 멤버를 찾고 통신하는 하부 라이브러리. k8s에서는 headless Service의 DNS로 서로를 발견한다.'],
...
캐시는 로컬 — **데모 전용**이고, 프로덕션 모드(`start`)는 아래 표의 답을 요구하며
답이 없으면 기동을 거부한다. 이 장은 그 답들을 하나씩 채운다.
...
| 수단 | 내용 | 판단 |
|---|---|---|
| **Keycloak Operator (공식)** | CR로 선언하면 StatefulSet·캐시 구성·업그레이드를 관리. realm import CR도 있다 | **기본 선택.** 운영 지식이 컨트롤러에 들어 있다 |
| Helm 차트 | codecentric의 keycloakx 등 커뮤니티 차트 | 차트 커스터마이징에 익숙하면. 공식 차트는 없다 |
| 직접 매니페스트 | StatefulSet + Service를 손으로 | 학습에는 좋다. 운영에서는 아래 설정을 전부 스스로 챙겨야 한다 |
...
flowchart LR
    subgraph K8S["k8s 클러스터"]
        ...
        I --> K1
        I --> K2
        K1 <-->|"Infinispan<br/>(JGroups · headless Svc)"| K2
    end
...
| 부품 | 역할 | 설정 포인트 |
|---|---|---|
| **Infinispan 클러스터** | replica 간 캐시·무효화 공유 | `--cache-stack=kubernetes` — headless Service의 DNS로 멤버 발견. Operator가 구성해 준다 |
...
- HA는 **Infinispan(kubernetes 스택) + DB 세션 + sticky** 세 부품 — 26부터는 재시작에도 로그인이 유지된다
