...
aliases: [Keycloak 오류, invalid audience, unknown kid, issuer did not match, redirect_uri, kubectl Unauthorized, kubectl Forbidden]
...
```d2 layout=elk pad=40 title="증상에서 출발해 로그인 화면 도달 여부와 토큰 claim 노출 여부로 갈라 연동 설정·AD·매퍼·전달 소비 구간 중 어디를 볼지 정하는 분기도"
direction: down
...
e1: "연동 설정 구간\nredirect URI · client · hostname\n(아래 표 A)" { class: warn }
e2: "AD 구간 (경계 ①)\nbind · LDAPS · sync\n(4장 표)" { class: warn }
q2: "Evaluate에서\n토큰에 보이나?" {
  shape: diamond
  class: key
}
e3: "매퍼 구간 (경계 ①·②)\nLDAP 매퍼 · protocol mapper\n(4장 · 3장)" { class: warn }
e4: "전달·소비 구간\noauth2-proxy · 앱 · API 서버\n(7장 사슬 · 아래 표 B)" { class: warn }
...
| **jwt 디코드** | 받은 토큰 payload를 base64url 디코드해 `iss`·`aud`·`exp`·claim을 직접 본다. 디코드는 서명 검증이 아니므로 민감한 실제 토큰을 외부 웹사이트에 붙여 넣지 않는다 |
...
| 404에 URL에 `/auth/`가 보임 | 옛 기본 경로를 복사했거나 서버의 `http-relative-path`와 불일치 | discovery URL을 열어 실제 base path 확인 ([0장](/keycloak/00-intro/)) |
...
| 간헐적으로 로그인·갱신 상태가 어긋남 | HA에서 cache invalidation·JGroups 클러스터링 실패 | Pod 로그의 cluster view, 노드 간 mTLS, `jdbc-ping`이 쓰는 공용 DB 연결 확인 ([8장](/keycloak/08-deploy/)) |
...
| API 서버 로그의 문구 | 뜻 | 해결 |
|---|---|---|
| `oidc: verify token: … expired` | 토큰 만료 — kubelogin refresh·캐시 문제 | 보통 자동 갱신된다. 안 되면 `kubectl oidc-login clean` 후 다시 로그인 |
...
| 인증은 되는데 `Forbidden` (403) | **인증은 성공, 인가 실패** — 401과 다른 문제다 | `kubectl auth can-i` + 그룹 prefix 확인. [CKA API 요청 처리 흐름](/cka/api-access/)의 영역 |
...
NetworkPolicy가 막고 있거나. 그 진단은 [CKA 트러블슈팅](/cka/troubleshooting/)의 영역이고,
경계를 넘나드는 감각 자체가 온프렘 관리자의 몫이다.

## 참고 자료

- [Keycloak hostname debug](https://www.keycloak.org/server/hostname) — issuer·proxy·frontend URL 계산을 `/realms/<realm>/hostname-debug`로 확인하는 방법
- [Kubernetes 1.35 인증 문서](https://v1-35.docs.kubernetes.io/docs/reference/access-authn-authz/authentication/) — OIDC 검증·CA·AuthenticationConfiguration 오류의 기준
- [kubelogin troubleshooting](https://github.com/int128/kubelogin) — token cache 확인과 `kubectl oidc-login clean`
