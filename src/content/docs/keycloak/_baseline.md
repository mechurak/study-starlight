...
Compose 실습의 환경 계약·자원·검증 범위는 [keycloak-lab 덱](/keycloak-lab/)의 `_baseline.md`가 정본이다.
이 덱의 학습 본문은 실습 결과를 인용할 때 해당 실습 페이지를 링크하고, 명령·경로·검증 현황을 다시 적지 않는다.
brokering·service account 같은 격리된 보조 실습의 재현 절차는 이 덱의 해당 장에 남긴다.
