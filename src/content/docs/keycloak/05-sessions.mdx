...
aliases: [SSO Session Idle, SSO Session Max, access token lifespan, refresh token rotation, offline token, logout, 토큰 회수]
...
```d2 layout=elk pad=40 title="realm 수준 SSO 세션 하나가 앱 A·B의 클라이언트 세션으로 갈라지고 각각 access token을 발급하는 계층"
direction: down
...
서명만 로컬 검증하는 일반적인 Resource Server에서는 **발급된 JWT access token을 즉시
회수할 수 없다**. Keycloak에 매 요청을 묻지 않기 때문이다 ([0장의 멘탈 모델](/keycloak/00-intro/)).
세션 종료는 새 토큰 발급과 refresh를 막지만, 이미 발급된 token은 `exp`까지 통과할 수 있다.
그래서 access token을 짧게 주고, 갱신 시점마다 세션과 현재 권한을 다시 판단한다.

:::caution["세션 종료 = 모든 access token 즉시 무효"가 아니다]
introspection을 매번 호출하거나 Keycloak adapter의 not-before push를 구현한 소비자는 더 빨리
거부할 수 있다. 하지만 kube-apiserver 같은 일반 OIDC JWT 검증자는 그런 Keycloak 전용 정책을
자동으로 알지 못한다. **범용적인 회수 보장은 짧은 `exp` + refresh 차단**까지다.
:::
...
:::caution[함정]
offline token은 **퇴사 처리의 사각지대**다. 일반 SSO 로그아웃·SSO 세션 종료로는 안 죽고,
AD 비활성화만으로 모든 소비자에서 즉시 막힌다고 가정해서도 안 된다.
...
```d2 pad=40 title="앱 A에서 로그아웃하면 Keycloak이 SSO 세션을 끝내고 앱 B에 backchannel logout을 통지해 각 앱의 세션이 폐기되는 순서"
...
k -> k: SSO 세션 종료
k -> b: "backchannel logout 통지 (서버 대 서버)" { style.stroke-dash: 3 }
b -> b: 자기 세션 폐기
k -> u: 로그아웃 완료 → 앱으로 redirect { style.stroke-dash: 3 }
```
...
| realm 전체 로그아웃 | Sessions → **Sign out all active sessions** | SSO 세션 종료 + 등록된 client에 logout 전파 시도 |
| 과거 토큰 revocation 기준 설정 | Sessions → **Revocation → Set to now** | Keycloak의 refresh·introspection과 정책을 전달받는 adapter에 적용. 일반 JWT 소비자는 자동 적용되지 않을 수 있음 |
| client 시크릿 교체 | Clients → Credentials → Regenerate | 이후 token 교환의 옛 시크릿 사용을 차단. 이미 발급된 access token은 별도 |
...
- [Keycloak Server Administration Guide — sessions, timeouts, offline access](https://www.keycloak.org/docs/latest/server_admin/) — 세션 수명·refresh rotation·offline token·revocation의 기준 동작
- [Keycloak 26 세션 저장 방식](https://www.keycloak.org/2024/12/storing-sessions-in-kc26) — persistent user sessions가 기본이 된 배경과 지원 구성

## 5장 요약

- 수명 설정은 전부 **탈취 위험 vs UX의 다이얼**이다 — 기본 전략은 "access token 짧게, 갱신은 세션으로"
- 층이 셋: **SSO 세션(realm) → 클라이언트 세션(앱) → access token(스냅샷)**
- 로컬 검증 access token은 즉시 회수가 보장되지 않는다 — **세션을 끊어 refresh를 막고 짧은 수명으로 잔여 시간을 제한**한다
