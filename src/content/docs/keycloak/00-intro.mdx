...
aliases: [Keycloak 입문, SSO 구조, IdP, AD 연동]
...
```d2 layout=elk pad=40 title="AD가 Keycloak을 거쳐 웹 앱과 쿠버네티스 API 서버로 이어지는 통역사 구도"
direction: right
...
ad: "AD\nLDAP · Kerberos의 세계" { class: mute }
kc: "Keycloak\n통역사 (IdP)" { class: key }
app: "웹 앱들\nOIDC · JWT의 세계" { class: mute }
k8s: 쿠버네티스 API 서버 { class: mute }
...
SSO 구조에서 비밀번호는 **사용자 브라우저가 Keycloak 로그인 화면에 제출하고,
Keycloak이 AD bind에 쓰는 경로에만** 있다. 업무 앱과 쿠버네티스에는 전달되지 않는다.
그들이 받는 것은 Keycloak이 개인키로 서명한 토큰(JWT)이고,
...
**2026년 8월** 기준이다. Keycloak은 26.x 안에서도 새 버전이 자주 나오고 옛 정보가 오래 남는 생태계라,
검색 결과를 볼 때 시점 확인이 특히 중요하다.

| 항목 | 지금 | 낡은 정보 (검색 결과에 많이 남아 있음) |
|---|---|---|
| Keycloak | **26.7** (2026-07) — Quarkus 기반 | WildFly 기반(16 이하), `standalone.xml` 설정 |
| URL 경로 | 기본값은 `https://호스트/realms/…` | `/auth/realms/…` — 17부터 **기본 경로에서 빠졌다**. `http-relative-path=/auth`로 일부러 복원한 환경은 예외 |
...
| k8s 쪽 연동 | **AuthenticationConfiguration 파일** (1.34에서 GA, 이 덱은 1.35 기준) | `--oidc-*` 플래그만 다루는 글 |

:::caution[함정]
옛 블로그의 예시 URL에는 `/auth`가 끼어 있다 (`…/auth/realms/corp/…`).
기본 설정의 지금 버전에 그대로 복사하면 404가 난다. 다만 운영자가 `http-relative-path=/auth`를
명시한 환경도 있으므로, 추측하지 말고 discovery 문서의 실제 `issuer`를 기준으로 삼는다.
:::

## 참고 자료

- [Keycloak 26.7.0 릴리스](https://www.keycloak.org/2026/07/keycloak-2670-released) — 기준 버전과 26.7의 preview 기능을 확인한다
- [Keycloak 릴리스 노트](https://www.keycloak.org/docs/latest/release_notes/index) — 26의 persistent sessions 기본 활성화와 버전별 변경을 확인한다
- [Kubernetes 1.35 인증 문서](https://v1-35.docs.kubernetes.io/docs/reference/access-authn-authz/authentication/) — 1.34에서 stable이 된 `AuthenticationConfiguration`의 현재 동작을 확인한다

## 0장 요약

- 대상은 **AD + Keycloak 환경을 설명·운영해야 하는 쿠버네티스 관리자**다
- 멘탈 모델 셋: **통역사 / 두 번의 매핑 / 서명으로 흐르는 신뢰**
- 기준 시점은 2026년 8월 (Keycloak 26.7 · k8s 1.35). `/auth` 접두사는 옛 기본값일 수 있으니 실제 discovery 문서로 확인한다
