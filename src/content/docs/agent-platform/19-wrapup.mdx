...
```d2 layout=elk pad=40 title="임직원과 IdP가 Portal·Catalog로 들어가 제품 중립 resource와 승인·배치를 거쳐 RuntimeAdapter가 세 runtime에 배포하고, 호출은 Stable Invoke API로 따로 들어가 Tool Policy와 감사로 이어지는 덱 전체 지도"
direction: down
...
user: 임직원 { class: key }
idp: "IdP\nuser · group" { class: key }
edge: "Stable Invoke API\nACL · session binding" { class: key }

portal: Portal · Catalog { class: zone }
core: "Agent · Knowledge · Tool · Version\nGrant · Publication" { class: zone }
policy: Approval · Placement { class: zone }
adapter: RuntimeAdapter { class: zone }

k0: "맨 Kubernetes\n온프렘 기준선" { class: ok }
kag: "kagent\n온프렘 Kubernetes" { class: ok }
acr: "AgentCore\nAWS managed runtime" { class: ok }

tool: Tool Policy · MCP { class: warn }
obs: Audit · OTel · Langfuse { class: warn }
...
kag -> tool
acr -> tool
kag -> obs: { style.stroke-dash: 3 }
acr -> obs: { style.stroke-dash: 3 }
```
