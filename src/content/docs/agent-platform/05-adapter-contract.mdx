...
> 좋은 adapter는 차이를 지우지 않고 **차이가 나타나는 한 곳을 만든다**
...
두 provider의 모든 기능을 공통 boolean으로 만들면 어느 쪽도 제대로 쓰지 못한다. target은 다음처럼 자신을 설명한다.
...
<LinkCard
  ...
  description="제품 중립 계약을 Kubernetes-native Agent resource로 번역하고 남는 빈칸을 확인한다."
  href="/agent-platform/06-kagent/"
/>
