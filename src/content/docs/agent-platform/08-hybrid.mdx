---
title: "8. 두 target을 함께 운영하기"
description: data zone·capability·cost·latency policy로 kagent와 AgentCore placement를 결정한다
---
...
| 질문 | 온프렘 쪽으로 기우는 답 | AgentCore 쪽으로 기우는 답 |
|---|---|---|
| data가 AWS에서 처리돼도 되는가 | 아니오 | 예 |
| private tool이 어디에 있는가 | 온프렘 저지연 필수 | VPC/DX 경로 허용 |
| 필요한 image architecture는 | amd64 전용 | ARM64 검증 가능 |
| session 격리 요구는 | Pod/sandbox면 충분 | session별 microVM 필요 |
| 운영 책임은 | Kubernetes team이 감당 | managed runtime 선호 |
| scale pattern은 | 안정된 상시 workload | 변동 큰 session workload |
| 장애 시 독립성은 | AWS 단절에도 실행 | AWS regional service 의존 허용 |
...
<LinkCard
  ...
  href="/agent-platform/09-operations/"
/>
