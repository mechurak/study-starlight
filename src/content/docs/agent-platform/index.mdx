...
kagent나 Amazon Bedrock AgentCore는 이 중 **실행 상태를 만드는 부분**을 잘 맡지만 전체 포털은 아니다.

이 덱은 먼저 제품 밖에 `Agent`·`AgentVersion`·`Grant`·`DeploymentTarget` 계약을 만든다. 그다음 같은 계약을
온프렘 Kubernetes의 kagent와 AWS 관리형 AgentCore에 각각 번역한다. 목표는 두 제품의 모든 기능을 똑같이 만드는
...
<DeckMap
  steps={[
    ...
    { label: '2~4장', href: '/agent-platform/02-domain-model/', title: '제품 밖의 계약', tone: 'zone',
      desc: 'Agent·Version·Grant·Deployment · 등록부터 폐기까지 · invoke와 tool action 권한',
    ...
    { label: '5장', href: '/agent-platform/05-adapter-contract/', title: 'Adapter contract', tone: 'ok',
      ...
      note: '두 backend의 차이를 숨기지 않으면서 호출부를 어떻게 안정시키는가' },
    { label: '6~7장', href: '/agent-platform/06-kagent/', title: 'kagent와 AgentCore', tone: 'warn',
      desc: 'Kubernetes-native self-managed runtime · AWS managed session runtime',
      note: '각 제품이 공통 계약의 어디까지 맡고 무엇을 남기는가' },
    { label: '8~9장', href: '/agent-platform/08-hybrid/', title: '하이브리드 운영', tone: 'bad',
      desc: 'data zone별 target 선택 · network · 감사 · SLO · 장애 격리와 복구',
      note: '온프렘과 AWS를 한 control plane에서 어떻게 운영하는가' },
    { label: '10~11장', href: '/agent-platform/10-adoption/', title: '도입과 마무리', tone: 'mute',
...
- 두 backend를 처음부터 같은 수준으로 구현하지 않는다. 하나를 기본 target으로 정하고 다른 하나는 규제·기능상
  필요한 Agent부터 추가한다.
...
**2026년 8월 18일 기준**이다. kagent v0.9의 인증·인가 경계와 AWS Agent Registry의 Preview 상태처럼
변화가 빠른 항목은 실제 도입 시 선택한 release와 region 문서를 다시 확인한다.

<LinkCard
  title="0. Runtime보다 먼저 볼 것"
  description="임직원이 Agent를 만들고 쓰는 전체 여정에서 kagent와 AgentCore가 맡는 자리를 고정한다."
