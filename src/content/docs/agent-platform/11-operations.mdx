...
> Agent가 답했다는 것보다 **어느 version이 누구의 권한으로 무슨 action을 했는지** 남는지가 중요하다
...
| 경로 | 대표 지표 | 실패의 첫 소유자 |
|---|---|---|
| 배포 | approval-to-ready, deploy failure, rollback time | control plane·adapter |
...
```text
agentId · versionId · deploymentId · targetId
principalId · sessionId · traceId · policyDecisionId
```
...
| Dapr workflow가 멈춤 | sidecar·Scheduler·workflow state store | 새 workflow instance를 중복 시작 |
...
kagent target은 cluster CPU·memory·Pod·DB와 local model capacity를 계산한다. Dapr Agents target은 app Pod와
sidecar, Scheduler, workflow state store의 처리량·보존량을 함께 본다. AgentCore target은 service quota,
...
adapter release, kagent CRD/controller, Dapr runtime·Agents SDK, AgentCore API contract를 Agent version과 분리해
...
- rollback과 기존 session 처리

수동 변경은 주기적 inventory와 desired-state diff로 찾는다. 보안 경계 drift는 자동 suspend가 가능해야 한다.

## Backup과 복구

가장 먼저 복구할 것은 provider resource가 아니라 platform DB의 Agent·Version·Grant·Publication과 audit key다.
artifact registry와 Git이 있으면 deployment는 adapter로 재생성할 수 있다.

복구 순서는 `IdP/ACL → catalog DB → artifact/secret reference → adapter → runtime → route → synthetic test`다.
...
<LinkCard
  ...
  description="기본 target 하나와 검증 Agent 두 개로 adapter·ACL·rollback의 가장 위험한 가정을 시험한다."
  href="/agent-platform/12-adoption/"
/>
