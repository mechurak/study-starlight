...
> 로그인했다는 사실은 **어느 Agent와 어느 업무 action을 허용할지** 답하지 않는다
...
IdP에서 사용자와 부서 group을 가져오되 ACL에는 불변 ID를 저장한다.

```text
principalType: USER | GROUP | WORKLOAD
principalId:   IdP sub 또는 object ID
...
`(principalId, agentId, conversationId, backendSessionId)` 관계를 보관하고 매 요청마다 소유권을 검사한다.
...
- email은 표시값이고 ACL key는 IdP의 불변 ID다.
...
<LinkCard
  ...
  description="제품 중립 version과 target을 kagent·Dapr Agents·AgentCore의 실제 resource로 번역하는 최소 interface를 만든다."
  href="/agent-platform/05-adapter-contract/"
/>
