...
```d2 layout=elk pad=40 title="experience·control·runtime·foundation 네 plane이 위에서 아래로 쌓이고, 사람의 요청은 control의 catalog를 거쳐 runtime으로 내려가며, 세 runtime이 foundation의 identity·모델·tool·관측을 공유하는 지도"
direction: down
...
run: "Runtime plane — 실행 backend" {
  ...
  dapr: "Dapr Agents · Workflow" { class: ok }
...
kagent에서는 Kubernetes와 controller가 Agent resource를 workload로 맞춘다. Dapr Agents에서는 일반 app Pod와
sidecar·Workflow·state store가 step 단위 복구를 맡는다. AgentCore에서는 AWS 관리형 runtime이 session별
microVM과 version endpoint를 맡는다. 공통 interface는 같아도 lifecycle·복구·격리 단위의 의미는 다르다.
