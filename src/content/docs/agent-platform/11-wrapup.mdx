...
1. kagent와 AgentCore는 platform 전체가 아니라 runtime adapter다.
...
11. data zone이 placement의 첫 hard constraint다.
12. stable company endpoint 뒤에서 publication만 target deployment를 바꾼다.

## kagent와 AgentCore 선택 카드
...
| hybrid | data 등급과 기능에 따라 두 조건이 실제 공존 | 두 adapter와 dependency graph, 제한된 failover 검증 |

둘 다 쓰는 것이 목표가 아니다. 회사 요구를 만족하는 default 하나를 정하고 secondary target의 존재 이유를
...
- [6. kagent adapter](/agent-platform/06-kagent/) · [7. AgentCore adapter](/agent-platform/07-agentcore/) — target 구현을 비교할 때
- [10. 두 Agent로 시작하기](/agent-platform/10-adoption/) — PoC acceptance test를 만들 때
