...
```mermaid
flowchart LR
    CORE["Platform Core<br/>Agent · Knowledge · Version · Grant"] --> PORT["RuntimeAdapter port"]
    CORE --> DPORT["Durability port<br/>ExecutionProfile"]
    PORT --> K0["KubernetesAdapter<br/>기준선"]
    PORT --> KA["KagentAdapter"]
    PORT --> AA["AgentCoreAdapter"]
    K0 --> K8S0["Deployment · Service · Job"]
    KA --> K8S["Agent CR · Service · Secret"]
    AA --> AWS["OCI/CodeZip · Runtime · Endpoint"]
    DPORT --> NONE["none<br/>현재"]
    DPORT -.-> DEFER["Temporal · Dapr Agents<br/>재개 시 후보"]
    K8S0 --> NORMAL["Normalized DeploymentStatus"]
    ...
    class CORE key
    class PORT,DPORT,NORMAL zone
    class K0,KA,AA,K8S0,K8S,AWS ok
    class NONE,DEFER zone
```
