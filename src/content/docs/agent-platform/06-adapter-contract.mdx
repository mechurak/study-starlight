...
flowchart LR
    ...
    class KA,AA,K8S,AWS ok
```

platform core는 Kubernetes client나 AWS SDK를 import하지 않는다. adapter가 provider credential, retry,
eventual consistency와 error message를 소유한다. 지금 adapter는 둘이지만 port는 개수를 가정하지 않는다 —
[보류한 durable execution 층](/agent-platform/12-durable-execution/)을 재개하면 같은 port에 세 번째
adapter가 붙는다.
...
| 공통 의도 | kagent target | AgentCore target |
|---|---|---|
| runtime workload identity | Kubernetes ServiceAccount | AgentCore workload/execution role |
| secret reference | Secret/Vault injection | Secrets Manager·AgentCore Identity provider |
| private CA | Secret volume·ModelConfig TLS | image trust store·private endpoint 설정 |
| egress policy | NetworkPolicy·proxy | security group·VPC endpoint·route |
