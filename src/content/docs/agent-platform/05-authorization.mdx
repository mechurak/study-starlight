...
```d2 layout=elk pad=40 title="사용자가 관리 권한 관문을 지나 포털로, 호출 권한 관문을 지나 runtime으로 가고, runtime이 다시 action 권한 관문을 지나 업무 API에 닿는 세 권한의 분리"
direction: down
...
portal: Portal control plane { class: warn }
runtime: Agent runtime { class: warn }
tool: 업무 API · data { class: warn }
