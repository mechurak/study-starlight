...
```d2 layout=elk pad=40 title="AgentVersion의 요구를 Placement Policy가 판정해 맨 Kubernetes·온프렘 kagent 후보·AgentCore 셋 중 하나에 배치하고, 셋 모두 IdP·ACL이 붙은 하나의 Stable Agent Endpoint로 다시 모여 Consumer에게 가는 흐름"
direction: down
...
spec: "AgentVersion\ndata · protocol · capability" { class: key }
place: Placement Policy {
  class: zone
  shape: diamond
}

k0: 맨 Kubernetes { class: ok }
kag: 온프렘 kagent 후보 { class: ok }
acr: AgentCore { class: ok }

route: Stable Agent Endpoint { class: ok }
idp: IdP · ACL { class: key }
user: Consumer { class: key }

spec -> place
place -> k0: restricted · code형 기준선
place -> kag: "restricted · 구성형/sandbox"
place -> acr: AWS allowed · session isolation
k0 -> route
kag -> route
acr -> route
idp -> route
