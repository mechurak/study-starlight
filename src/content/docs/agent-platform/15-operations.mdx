...
```d2 layout=elk pad=40 title="Portal·ACL에서 Runtime, Model, Tool로 이어지는 요청 경로와 네 구간이 모두 Audit·Trace로 점선으로 흘려보내는 기록"
direction: right
...
edge: Portal · ACL { class: key }
rt: Runtime { class: ok }
model: Model { class: ok }
tool: Tool { class: ok }
audit: Audit · Trace { class: warn }

edge -> rt
rt -> model
model -> tool

edge -> audit: { style.stroke-dash: 3 }
rt -> audit: { style.stroke-dash: 3 }
model -> audit: { style.stroke-dash: 3 }
tool -> audit: { style.stroke-dash: 3 }
```
...
<LinkCard
  ...
  description="kagent와 같은 생태계의 트래픽 층이 이 덱의 세 권한 경계 중 무엇을 맡을 수 있고 무엇을 맡지 못하는지 확인한다."
