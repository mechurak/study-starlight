...
```d2 layout=elk pad=40 title="Unity Catalog가 승인한 테이블·모델이 SQL warehouse·노트북·ML 학습으로 갈라져 대시보드·앱·모델 서빙으로 이어지고, row·column 정책이 카탈로그에 걸리는 소비 경로"
direction: down
...
uc: "Unity Catalog\n승인 table · view · model" { class: zone }
sql: SQL warehouse { class: ok }
nb: Notebook · Spark · Python { class: ok }
ml: ML training · MLflow { class: ok }
bi: Dashboard · BI tool { class: key }
app: SQL/API consumer { class: key }
exp: 탐색 · data science { class: key }
serve: Model serving { class: key }
policy: "row · column policy\naudit · lineage" { class: zone }
