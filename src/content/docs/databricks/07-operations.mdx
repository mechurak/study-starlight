...
```d2 layout=elk pad=40 title="SLA·소유자·예산 정의에서 실행·관측·조치·리뷰를 거쳐 다시 정의로 돌아오는 운영 순환"
direction: down
...
define: SLA · owner · budget { class: key }
run: job · warehouse · serving { class: ok }
obs: freshness · failure · latency · cost { class: warn }
act: retry · scale · stop · alert { class: warn }
review: capacity · policy · architecture review { class: zone }
