...
```d2 layout=elk pad=40 title="이용자 정책·GPUStack 관리와 라우팅·모델 실행·DGX Spark fleet 네 층이 위에서 아래로 이어지고, pair cell은 vLLM만 쓰는 덱 전체 지도"
direction: down
...
north: 이용자 정책 {
  user: 사용자 · 앱 { class: key }
  lite: "LiteLLM\nkey · quota · alias · fallback" { class: key }

  user -> lite
}

plane: GPUStack 관리 · 라우팅 {
  route: Model Route { class: zone }
  ctrl: Server · Scheduler · Controller { class: zone }

  route -> ctrl: { style.stroke-dash: 3 }
}
...
fleet: DGX Spark fleet {
  single: "single cells\nspark-05 … 10" { class: ok }
  pair: "pair cells\nspark-01+02 · 03+04" { class: warn }
}

north.lite -> plane.route
plane.route -> run.v1
plane.route -> run.f1
plane.ctrl -> run.v1
plane.ctrl -> run.f1
