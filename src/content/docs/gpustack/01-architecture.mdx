...
```d2 layout=elk pad=40 title="사용자에서 LiteLLM·Gateway를 거쳐 모델 API에 닿는 요청 경로와, 운영자에서 Server·Scheduler·Worker를 거쳐 Docker 런타임에 닿는 제어 경로를 위아래로 나눈 그림"
direction: down
...
request: 요청 경로 {
  ...
  gw: GPUStack AI Gateway { class: zone }
  api: vLLM 또는 FastAPI { class: ok }
...
control: 제어 경로 {
  admin: 운영자 { class: zone }
  server: GPUStack Server { class: zone }
  sched: Scheduler · Controller { class: zone }
  worker: GPUStack Worker { class: zone }
  docker: Docker + NVIDIA Runtime { class: ok }
...
control.docker -> request.api: { style.stroke-dash: 3 }
```
