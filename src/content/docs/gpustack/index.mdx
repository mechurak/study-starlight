...
<DeckMap deck="gpustack" />

## 전체를 관통하는 두 경계

```d2 layout=elk pad=40 title="이용자 요청이 LiteLLM과 GPUStack AI Gateway를 거쳐 deployment로 가고, vLLM과 FastAPI custom backend 두 갈래가 다시 DGX Spark worker에서 실행되는 경로"
direction: down
...
user: 이용자 · 사내 서비스 { class: key }
lite: "LiteLLM\n키 · quota · alias · fallback" { class: key }
route: "GPUStack AI Gateway\nmodel route · load balancing" { class: zone }
deploy: "GPUStack deployment\n배치 · replica · 재시작" { class: zone }
vllm: "vLLM\nLLM · embedding" { class: ok }
fast: "FastAPI custom backend\nclassification · embedding" { class: ok }
spark: DGX Spark worker { class: ok }
