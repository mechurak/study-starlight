...
```d2 layout=elk pad=40 title="서비스 지표·모델 지표·노드 지표 세 갈래가 하나의 사용자 영향 판단으로 모이는 관측 구조"
direction: down
...
svc: "서비스 지표\n요청 · 오류 · 지연 · queue" { class: key }
model: "모델 지표\nTTFT · tokens/s · batch · cache" { class: ok }
node: "노드 지표\nmemory · temperature · disk · network" { class: warn }
decide: 사용자 영향 판단 { class: key }
