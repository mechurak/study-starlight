...
```mermaid
flowchart TB
    subgraph prepare["A · 노드를 준비한다"]
        OP["GPU Operator"] --> STACK["driver · toolkit · device plugin<br/>GFD · MIG · DCGM"]
        STACK --> GPU["Spark · A100 · B300 node"]
    end
    subgraph deploy["B · 모델을 배포한다"]
        GIT["Git"] --> ISVC["InferenceService"] --> KS["KServe controller"]
        RT["ServingRuntime"] --> KS
        KS --> KOBJ["Deployment · Service · HTTPRoute"]
        KOBJ --> POD["vLLM · FastAPI Pod"]
    end
    subgraph request["C · 요청이 지나간다"]
        APP["사내 앱"] --> LITE["LiteLLM"] --> GW["Gateway · Service"] --> POD
        POD --> GPU
    end
    ...
    class GIT,RT,APP,LITE key
    class ISVC,KS,GW zone
    class KOBJ,POD ok
    class OP,STACK,GPU warn
```
...
```mermaid
flowchart TD
    START["새 model serving"] --> FIT{"한 Pod · 한 node에<br/>model이 들어가는가?"}
    FIT -->|"예"| STD["Standard InferenceService"]
    FIT -->|"아니오"| MULTI["LLMInferenceService<br/>LeaderWorkerSet 검토"]
    STD --> SLO{"실제 부하에서<br/>SLO를 만족하는가?"}
    SLO -->|"예"| KEEP["Standard 유지"]
    SLO -->|"아니오 · routing 병목"| EPP["Inference Extension · EPP 검토"]
    SLO -->|"아니오 · prefill 간섭"| PD["prefill · decode 분리 검토"]

    classDef ok fill:#dcfce7,stroke:#16a34a,color:#14532d
    classDef warn fill:#fef3c7,stroke:#d97706,color:#78350f
    classDef zone fill:#ede9fe,stroke:#7c3aed,color:#4c1d95
    class START,STD,KEEP ok
    class FIT,SLO warn
    class MULTI,EPP,PD zone
```
