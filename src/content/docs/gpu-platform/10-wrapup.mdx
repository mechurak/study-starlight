...
| [4A](/gpu-platform/04a-serving-operations/) | GPU utilization보다 **queue·TTFT·ITL·KV cache와 이용자 SLO**를 먼저 본다 |
...
6. **serving 안전선** — 실제 prompt·output 분포로 SLO·안전 동시성·queue 상한·timeout budget을 정했다.
7. **admission** — Kueue flavor·quota·borrowing·priority와 pending reason을 시험했다.
8. **distributed lifecycle** — JobSet·Trainer·LWS의 gang start·failure·checkpoint를 검증했다.
9. **interactive parity** — Notebook·workspace·storage·idle·portal·usage 경로를 대표 사용자와 확인했다.
10. **data와 network** — cold start budget·cache·checkpoint·RDMA·Gateway 경계를 측정했다.
11. **rollback** — model endpoint와 A100 node 하나를 실제로 기존 환경으로 되돌렸다.
12. **B300 준비** — 도착 전 driver·image·full-node·fabric·burn-in 기준을 승인했다.
13. **Backend.AI 종료 gate** — active session·data·사용자·audit가 남지 않았음을 확인했다.
...
이용자 오류율과 vLLM waiting·queue time을 먼저 맞춘다.

TTFT·ITL·KV cache·preemption으로 prefill과 decode를 나눈다.

포화면 긴 요청과 retry부터 줄이고 deadline만 늘리지 않는다.
</Card>
