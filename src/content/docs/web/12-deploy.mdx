...
<TermIntro
  terms={[
    ['serverless', 'Serverless', '상시 서버 없이 요청이 올 때만 함수가 깨어나 실행되는 모델. 서버가 없는 게 아니라 **서버 관리가 없는** 것이다.'],
    ['콜드 스타트', 'Cold Start', 'serverless 함수가 잠들어 있다가 첫 요청에 깨어나며 생기는 지연. 이 모델의 대표 대가다.'],
...
flowchart TB
    ...
    Q1 -->|"있음"| Q2["상시 프로세스가 필요한가?<br/>웹소켓 · 크론 · 큐 · 긴 작업"]
...
한 서비스가 여러 형태를 섞는 게 보통이다 — Next.js를 Vercel에 올리면 정적 페이지는
CDN으로, 서버 코드는 serverless 함수로 **알아서 쪼개진다.** 상시 프로세스가 필요한
API 서버(10장의 경로 ②)만 PaaS에 따로 올린다.

:::caution[함정]
serverless의 제약은 계약서의 작은 글씨다 — **실행 시간 제한, 상시 연결(웹소켓) 불가,
콜드 스타트, 그리고 DB 커넥션 고갈**(함수가 수백 개 깨어나면 각자 커넥션을 잡는다 —
그래서 커넥션 풀러를 사이에 둔다). "왜 NestJS는 Vercel에 못 올리지?"의 답이 전부 여기 있다.
:::
...
4. 문제가 생기면 **롤백** — 이전 배포가 통째로 보존돼 있어(불변 배포) 클릭 한 번에 돌아간다
...
- 로컬은 `.env` 파일 — **반드시 `.gitignore`에** 넣는다. 커밋된 비밀 키는 이미 유출된 것이다
...
| 성능 | 실사용자 체감 속도는 (Core Web Vitals) | Vercel Analytics, Lighthouse |
...
- 한 서비스가 형태를 섞는다 — 프론트는 Vercel(정적+함수), 상시 서버는 PaaS
- serverless의 작은 글씨 — 시간 제한 · 상시 연결 불가 · 콜드 스타트 · 커넥션 고갈
- 워크플로는 **git push = 배포** — 프리뷰로 리뷰하고, 불변 배포라 롤백이 클릭 한 번
