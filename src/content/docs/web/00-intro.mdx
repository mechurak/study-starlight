...
```d2 layout=elk pad=40 title="새 도구를 만났을 때 어느 층의 물건인지, 무슨 통증을 풀었는지, 같은 층의 기존 도구와 무엇이 다른지를 차례로 묻는 세 단계"
direction: down
...
q: 새 도구를 만났을 때 { class: mute }
l: 어느 층의 물건인가? { class: key }
p: 무슨 통증을 풀었는가? { class: ok }
r: "같은 층의 기존 도구와\n무엇이 다른가?" { class: ok }
