...
```d2 layout=elk pad=40 title="HTML이 도착해 화면은 보이지만 JS를 다운로드·실행해 하이드레이션이 끝나야 상호작용이 가능해지는 세 단계"
direction: right
...
```d2 layout=elk pad=40 title="검색 노출과 첫 화면 속도가 중요하지 않으면 CSR, 중요하면 내용이 사용자마다 다른지로 SSR과 정적 계열을 가르고, 정적 계열은 다시 바뀌는 빈도에 따라 SSG와 ISR로 나뉘는 선택 흐름"
direction: down
...
q1: "검색 노출이나\n첫 화면 속도가 중요한가?" { class: mute }
q2: "내용이 사용자마다\n다른가?" { class: mute }
q3: 자주 바뀌는가? { class: mute }
csr: "CSR (SPA)\nVite + React 등" { class: key }
ssr: "SSR\n(+ 페이지 일부는 정적으로)" { class: key }
ssg: "SSG\n블로그 · 문서 · 랜딩" { class: ok }
isr: "ISR\n주기적 재생성" { class: warn }
