...
<TermIntro
  terms={[
    ...
    ['SPA', 'Single-Page Application', 'HTML 한 장으로 시작해 이후 화면 전환을 전부 JS가 처리하는 앱. CSR과 짝을 이루는 말이다.'],
...
지금의 프레임워크들은 이 갈래를 **페이지 단위로 골라 쓰게** 해 준다.
...
- **대가** — 첫 화면까지 오래 걸린다. 검색 엔진 크롤러가 빈 HTML을 본다(SEO 불리)
...
- **대가** — 내용을 바꾸려면 다시 빌드해야 한다. 사용자별 페이지는 못 만든다
