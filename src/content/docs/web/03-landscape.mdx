...
```d2 layout=elk pad=40 title="프론트엔드·백엔드·DB로 나누던 전통적 구분과, Next.js가 화면과 서버 코드를 함께 맡고 Supabase가 DB·인증·파일·실시간을 맡으며 별도 백엔드는 필요해질 때 두는 2026년의 현실을 나란히 놓고 경계가 흐려졌음을 표시한 그림"
direction: down
...
m: 2026년의 현실 {
  ...
  m1: "Next.js\n화면 + 서버 코드까지" { class: key }
  m2: "Supabase\nDB + 인증 + 파일 + 실시간" { class: ok }
  m3: "별도 백엔드\n필요해지면 그때" { class: mute }
...
f -> m: 경계가 흐려짐 { style.stroke-dash: 3 }
```
