...
```d2 layout=elk pad=40 title="브라우저 런타임과 Node.js 런타임이 똑같이 V8 엔진을 쓰지만, 붙어 있는 것이 각각 DOM·fetch·localStorage와 파일 시스템·네트워크·프로세스로 다르다는 대비"
direction: down
...
b: 브라우저 런타임 {
  b1: V8 엔진 { class: key }
  b2: DOM · fetch · localStorage { class: ok }
...
다만 현대 프론트엔드의 도구 사슬은 대개 **Node 호환 런타임** 위에서 돈다.
서버를 다른 언어로 짜도 Node·Bun·Deno 중 하나를 빌드 도구 실행 환경으로 만날 가능성이 높다.
...
- Node 호환 범위가 넓어졌지만 모든 패키지가 똑같이 동작한다고 가정하지 말고 테스트한다
...
- 새 코드는 **ESM을 기본 선택**으로 본다. 다만 기존 Node 생태계와 일부 도구에는 CJS가 남아 있다
...
- [Node.js 릴리스 현황](https://nodejs.org/en/about/previous-releases) — Node 24 LTS·26 Current와 프로덕션 권고
- [Node.js TypeScript 실행](https://nodejs.org/api/typescript.html) — 내장 type stripping의 범위와 제약
