...
```d2 layout=elk pad=40 title="코드 배송 시스템이라는 정의에서 소유권과 관리 책임, 테마라는 관리 단위, 다섯 리소스와 두 벌, 레지스트리 배포까지 이어지는 덱 요약"
direction: down
...
a: "shadcn/ui\n= 코드 배송 시스템\n컴포넌트가 내 레포로 온다" { class: mute }
b: "소유권을 얻는다\n= 관리 책임이 생긴다" { class: warn }
c: 관리의 단위 = 테마 { class: key }
d: "색 · 형태 · 글자\n컴포넌트 · 아이콘/폰트" { class: ok }
e: 라이트/다크 두 벌 { class: mute }
f: "레지스트리로 배포\n= 여러 앱이 한 출발점" { class: mute }
