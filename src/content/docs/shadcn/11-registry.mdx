...
<TermIntro
  terms={[
    ...
    ['`--diff`', '', '내가 고친 파일과 업스트림 최신본의 차이를 보여주는 CLI 옵션. 소유권 모델의 유일한 안전장치.'],
...
| 종류 | 담기는 것 | 언제 |
|---|---|---|
| **`registry:theme`** | **CSS 변수 한 벌** | **팀 테마를 여러 앱에 뿌릴 때 — 가장 흔하다** |
...
4. **다른 앱에서 받는다**
   ...
   그 앱의 `globals.css`에 토큰이 **병합된다.** 기존 값은 덮어써진다.

</Steps>

:::tip[핵심]
**테마를 레지스트리로 만들면 브랜드 변경이 "앱마다 CSS 고치기"에서 "버전 올리기"가 된다.**
디자인이 바뀌면 레지스트리를 갱신하고, 각 앱은 명령 한 줄로 따라온다.
:::
...
레지스트리와 별개로, **소유권 모델의 유일한 안전장치**가 이것이다.
...
내 파일에는 이 개선이 없다 — `--diff` 없이는 알 방법이 없다.
...
- **`--diff`는 소유권 모델의 유일한 안전장치**다. 업스트림 접근성 수정을 이걸로만 알 수 있다
