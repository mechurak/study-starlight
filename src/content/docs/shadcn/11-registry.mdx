...
```d2 layout=elk pad=40 title="같은 공식 컴포넌트를 받은 앱 A와 앱 B가 각자 수정하다 6개월 뒤 같은 이름의 다른 물건이 되는 드리프트"
direction: down
...
s: shadcn 공식 { class: mute }
a: "앱 A\nbutton.tsx" { class: mute }
b: "앱 B\nbutton.tsx" { class: mute }
a2: "success variant 추가\n기본 크기 sm" { class: warn }
b2: "로딩 상태 추가\nradius 다르게" { class: warn }
x: "6개월 뒤\n같은 이름, 다른 물건\n= 드리프트" { class: bad }
