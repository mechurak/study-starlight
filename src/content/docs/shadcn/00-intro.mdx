...
```d2 layout=elk pad=40 title="테마가 색·형태·글자·컴포넌트·아이콘 폰트 다섯 갈래로 나뉘고 각각 4~8장이 맡는 지도"
direction: right
...
| shadcn CLI | **4.x** | preset·template과 `--dry-run`·`--diff`·`--view`가 한 흐름으로 정리됐다 |
| Next.js | 16 | 이 덱은 Next.js에 크게 의존하지 않는다. Vite 등에서도 대부분 그대로다 |
| 기반 라이브러리 | **Base UI** | 2026년 7월부터 기본값. 예전 기본값은 Radix UI였다 |

## 참고 자료

- [shadcn CLI v4 발표](https://ui.shadcn.com/docs/changelog/2026-03-cli-v4) — 이 덱에서 쓰는 CLI 명령과 preset·template 기준
- [Base UI 기본값 전환](https://ui.shadcn.com/docs/changelog/2026-07-base-ui-default) — 새 프로젝트의 기본 기반과 Radix 지원 상태
