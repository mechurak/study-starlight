...
```d2 layout=elk pad=40 title="원시 토큰 → 의미 토큰 → 컴포넌트 토큰 3단 계층"
direction: down
...
a: "1. 원시 토큰 (primitive)\n--indigo-600: oklch(0.51 0.23 277)\n순수한 값 · 의미 없음" { class: mute }
b: "2. 의미 토큰 (semantic)\n--primary: var(--indigo-600)\n역할을 부여한다" { class: key }
c: "3. 컴포넌트 토큰 (component)\n--button-bg: var(--primary)\n특정 컴포넌트 전용" { class: warn }
...
```d2 layout=elk pad=40 title="바탕 토큰과 foreground 토큰을 짝으로 함께 써야 대비가 유지된다"
direction: down
...
m: "--muted\n바탕" { class: key }
mf: "--muted-foreground\n그 위의 글자" { class: mute }
r: "항상 함께 쓴다\nbg-primary text-primary-foreground ✅" { class: ok }
x: "한쪽만 바꾸면\n대비가 깨진다 ❌" { class: bad }
...
```d2 layout=elk pad=40 title="CSS 변수 정의에서 @theme inline 유틸리티 생성을 거쳐 컴포넌트 클래스로 이어지는 경로"
direction: down
...
아래 두 카드는 **같은 컴포넌트**다 — 렌더 직전에 값만 갈렸다.

<ThemeCompare themes={['shadcn', 'dark']} labels={[':root 값', '.dark 값 — 마크업 동일']} compact />
...
<ThemeCompare themes={['corporate', 'shadcn', 'material']} labels={['--radius: 0.125rem', '--radius: 0.5rem', '--radius: 1.25rem']} compact />
