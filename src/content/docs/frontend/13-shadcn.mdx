...
import GalleryDemo from '../../../components/demos/GalleryDemo.astro';
...
shadcn/ui만 따로 파고드는 [**shadcn/ui 덱**](/shadcn/)이 있다.
용어를 하나씩 풀어 쓰고 테마 중심으로 재구성한 자료라, 이 장이 빡빡하게 느껴지면 그쪽이 편하다.
→ [1. shadcn/ui란 무엇인가](/shadcn/01-what-is-shadcn/)
:::
...
```d2 layout=elk pad=40 title="내 앱에서 shadcn 복사 코드와 Base UI를 거쳐 브라우저 DOM에 닿는 4층"
direction: down
...
b: "components/ui/button.tsx\n= shadcn 이 복사해 준 코드\nTailwind 클래스 + cva" { class: warn }
c: "Base UI (또는 Radix / React Aria)\n포커스 트랩 · 키보드 · ARIA · 포지셔닝" { class: key }
d: 브라우저 DOM { class: mute }
