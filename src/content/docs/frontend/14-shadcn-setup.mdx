...
```d2 layout=elk pad=40 title="shadcn init에서 cssVariables·baseColor·style 선택이 나중에 못 바꾸는 결정으로 이어지는 갈래"
direction: down
...
good: "토큰 기반\n테마 교체 가능" { class: ok }
nope: "하드코딩 색\n17장 불가" { class: bad }
b: "baseColor 선택\nzinc · slate · stone …" { class: mute }
c: "style 선택\n8가지" { class: mute }
l: "🔒 나중에 못 바꿈" { class: warn }
l2: "🔒 나중에 못 바꿈" { class: warn }
