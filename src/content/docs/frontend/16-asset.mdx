...
```d2 layout=elk pad=40 title="components/ui → components/shared → 도메인 컴포넌트 3단 배치"
direction: down
...
a: "components/ui/\nshadcn 원본 — 되도록 손대지 않는다\nButton · Card · Dialog · Input…" { class: key }
b: "components/shared/\n우리 확장 — 프로젝트 공통\nPageHeader · EmptyState · DataTable · ConfirmDialog" { class: ok }
c: "app/(app)/**/_components/\n도메인 컴포넌트 — 이 화면 전용\nInvoiceRow · MemberInviteForm" { class: warn }
...
```d2 layout=elk pad=40 title="자체 레지스트리 흐름 — 컴포넌트 소스에서 빌드·정적 호스팅을 거쳐 다른 프로젝트가 설치한다"
direction: down
...
a: "registry/\n컴포넌트 소스" { class: mute }
b: "registry.json\n목록 정의" { class: mute }
c: "shadcn build\n→ public/r/*.json" { class: ok }
d: "정적 호스팅\nregistry.acme.com" { class: mute }
e: "다른 프로젝트\nshadcn add @acme/empty-state" { class: key }
