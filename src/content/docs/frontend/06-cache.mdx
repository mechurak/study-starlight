...
```d2 layout=elk pad=40 title="CDN에서 즉시 오는 정적 껍데기와 요청 시 스트리밍되는 사용자 설정"
direction: down
...
shell: "정적 껍데기 — CDN 에서 즉시" {
  a: "헤더 · 내비게이션\n순수 계산" { class: ok }
  b: "블로그 목록\n'use cache'" { class: key }
  c: "사용자 설정 자리\nSuspense fallback" { class: warn }
}
d: "cookies() 읽어\n실제 설정 표시" { class: warn }

shell.c -> d: 요청 시 스트리밍 { style.stroke-dash: 3 }
```
