...
> 과거 이관 기록이다. 아래 표시 번호·order 예약은 당시 결과를 설명한다.
> 새 작업은 [콘텐츠 작성 규칙](../content-authoring.md)의 번호 없는 명명과
> [검증 지침](../verification.md)의 최소 검증을 따른다. 과거 예약을 새 번호 규칙으로 적용하지 않는다.

2026-09-12 원본 커밋 `32c1012` 기준. [실행 계획](0001-cka-udemy-rework.md)의 M0 산출물이다.
...
| 실패 분기 — 이미지가 안 당겨질 때 | 학습 보충 | `11-helm` — 이미지 이전 실패 — 랩 환경 보충 |
...
위 12-kustomize 표는 실제 목적 제목으로 갱신했다. 당시 M2~M8 원본 표는 예약 상태였고,
최종 목적지는 아래 M8 감사에서 다시 확인했다.
...
## M8 최종 이관 감사 (2026-09-13)

- 원본 기준 커밋 `a125ab6`의 12개 본문 h2/h3 227개를 이 문서의 12개 표와 다시 대조했다.
  모든 절은 유지·이동·통합·학습 보충 중 하나로 처리되어 있고, 표에 적힌 목적 slug는 모두 존재한다.
- 최종 실습 덱은 1~39장, order 1010~1390이다. 번호·order는 연속이고 중복이 없으며 39개
  페이지가 모두 `<Thesis>`와 `_deck.mjs`의 탐색 링크를 가진다.
- 같은 원본 기준의 외부 URL 271개와 최종 덱 전체를 집합 비교해 누락 0개를 확인했다.
  기존 12개 URL은 각 원래 주제의 대표 작업에 유지했다. 저장소 내부에서 이동한 앵커는 새 slug로
  갱신했지만 외부 북마크의 모든 옛 앵커 호환까지 보장하지는 않는다.
- 커리큘럼 27항목의 표는 최종 slug와 일치한다. HA control plane 전체 구축, CSI 드라이버 설치,
  독립 operator 종합 설치, probes 전용 실습, LoadBalancer 외부 IP, Helm/Kustomize 기반
  클러스터 컴포넌트 설치는 index에도 실습 공백으로 표시했다.
- `exam-search`는 38개 작업 페이지를 첫 관찰 명령·공식 검색어와 연결한다. 상세 명령·실패 분기·
  완료 판정은 각 목적 페이지를 정본으로 두어 시험 색인에 다시 복제하지 않는다.
