...
랜딩의 hero 소개는 없애고 태그 필터부터 표시한다. Starlight이 대신 만드는 제목 패널은
`custom.css`에서 시각적으로 숨겨 접근성·본문 바로가기용 h1을 유지한다.
필터는 사이트 헤더 아래에 sticky로 두며, 화면이 낮으면 필터 안에서 스크롤할 수 있다.
결과가 줄어도 필터가 밀려 올라가지 않도록 카탈로그에 최소 화면 높이를 확보하고,
카탈로그의 스크롤 앵커 보정을 끈다. 태그 개수는 두 자리 폭을 확보해 선택 시 줄바꿈을 유지한다.
