...
cd "$lab_directory"
"$script_directory/status.sh" >"$verification_directory/status-before.txt"
...
"$script_directory/reset.sh" --dry-run >"$verification_directory/reset-dry-run.txt"
if "$script_directory/reset.sh" >"$verification_directory/reset-guard.txt" 2>&1; then
...
stack_stopped=false
"$script_directory/status.sh" >"$verification_directory/status-after.txt"
