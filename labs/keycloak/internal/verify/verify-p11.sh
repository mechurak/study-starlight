#!/bin/sh
set -eu
# Historical destructive full-reproduction verification.
...
"$lab_directory/scripts/status.sh" >/dev/null
...
"$lab_directory/scripts/reset.sh" --dry-run >"$temporary_directory/reset-dry-run.txt"
"$lab_directory/scripts/reset.sh" --confirm "$confirmation_phrase" >"$temporary_directory/reset.txt"
...
for reset_name in p03 p05 p06 p07 p08 p09 p10 p11 d09 d16 d18 d24 guided; do
  if [ -e "$state_directory/verification/$reset_name" ]; then
    echo "Compose verification state remained after confirmed reset: $reset_name" >&2
    exit 1
  fi
done
...
if ! "$lab_directory/scripts/first-start.sh" >"$temporary_directory/first-start.txt" 2>"$temporary_directory/first-start.stderr.txt"; then
...
"$lab_directory/scripts/status.sh" >"$verification_directory/status-after-first-start.txt"
record_environment
...
stack_stopped=false
"$lab_directory/scripts/status.sh" >"$verification_directory/status-after-resume.txt"
compose --profile p07 run --pull never --rm --no-deps p07-diagnostic \
