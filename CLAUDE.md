...
`shadcn` · `web` · `keycloak` · `kafka` · `server` · `starlight` · `onprem` · `observability` · `rl` 덱은 **각 장 첫머리의 `<TermIntro>` 상자가
...
- **한 문단(또는 표 한 칸)에 물결표 `~`를 두 번 쓰지 않는다** — GFM이 그 사이를 **취소선으로
  먹는다.** `제어 50~200Hz, **정책은 20~50Hz**` 처럼 범위를 둘 쓰면 사이가 `<del>`이 되고
  안에 있던 `**`까지 깨진다. 빌드는 통과한다. 한쪽을 "수십 분에서 몇 시간"처럼 말로 풀면 된다.
  판정도 산출물로 — `pnpm build` 후 `grep -rho '<del>[^<]*</del>' dist --include=index.html | sort -u`
  (`~~취소선~~`으로 일부러 쓴 것만 남아야 정상이다).
